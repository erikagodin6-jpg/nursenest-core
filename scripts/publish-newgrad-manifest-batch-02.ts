#!/usr/bin/env npx tsx
/**
 * Publishes new-grad nursing production batch 02 from
 * ../../data/blog-content/newgrad-prod-batch-02/ (repo root) into BlogPost (Prisma).
 *
 * Usage (from this package directory):
 *   npx tsx scripts/publish-newgrad-manifest-batch-02.ts
 *   npx tsx scripts/publish-newgrad-manifest-batch-02.ts --dry-run
 *
...
/** Outer repo root (parent of this Next.js app). */
const REPO_ROOT = path.join(__dirname, "..", "..");
const BATCH_DIR = path.join(REPO_ROOT, "data/blog-content/newgrad-prod-batch-02");
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    const data: Prisma.BlogPostCreateInput = {
      ...
      apaReferences: APA,
      legacySource: "newgrad-prod-batch-02",
