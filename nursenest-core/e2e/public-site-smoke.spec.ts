 ...
 *
 * Run: `npx playwright test e2e/public-site-smoke.spec.ts`
 */
...
function attachObservers(page: Page): Observers {
  ...
  const onRequestFailed = (req: import("@playwright/test").Request) => {
    const url = req.url();
    if (shouldIgnoreFailedRequest(url)) return;
...
test.describe("Public site smoke", () => {
  test("homepage loads", async ({ page }, testInfo) => {
    ...
    const d = await assertDiagnostics(o, testInfo.title);
    o.dispose();
