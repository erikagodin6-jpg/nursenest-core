...
function makeBowtie(s: ScenarioTemplate): CnplePracticalQuestion {
  ...
  return {
    ...
    options: [
      ...
      { id: "condition-distractor", text: "Expected baseline variation", correct: false, rationale: `This may be tempting if ${s.expectedFinding} is considered alone, but ${s.concerningCue} represents a change from baseline that better supports ${s.condition}.` },
      { id: "action-correct", text: s.priorityAction, correct: true, rationale: `${s.priorityAction} addresses ${s.concerningCue} within practical nursing scope while ensuring the regulated team is updated promptly.` },
      { id: "action-unsafe", text: s.unsafeAction, correct: false, rationale: `${s.unsafeAction} misses the clinical risk suggested by ${s.concerningCue} and could delay reassessment or team notification.` },
      { id: "monitor-correct", text: s.monitor, correct: true, rationale: `${s.monitor} is the best follow-up because it shows whether ${s.condition} is stabilizing or worsening after the intervention.` },
    ...
    correctAnswer,
    rationale: baseRationale(s, {
      condition: `${s.condition} is correct because ${s.concerningCue} is a change from the expected finding: ${s.expectedFinding}.`,
      action: `${s.priorityAction} is correct because it addresses the current risk while staying within practical nursing scope and local escalation pathways.`,
      monitoring: `${s.monitor} is correct because reassessment confirms response and identifies worsening before the next routine round.`,
    }),
    hints: hints(s),
...
function makeChartReview(s: ScenarioTemplate): CnplePracticalQuestion {
  ...
  return {
    ...
    options: [
      { id: "A", text: s.concerningCue, correct: true, rationale: `${s.concerningCue} requires immediate follow-up because it suggests ${s.condition} rather than the expected finding of ${s.expectedFinding}.` },
      { id: "B", text: s.expectedFinding, correct: false, rationale: "This is closer to baseline and does not explain acute risk." },
      { id: "C", text: "The client prefers tea with breakfast.", correct: false, rationale: "Food and beverage preference supports comfort planning, but it does not explain the acute physiologic change in the chart." },
