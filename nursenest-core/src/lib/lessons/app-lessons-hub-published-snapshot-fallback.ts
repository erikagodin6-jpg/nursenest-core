...
export function appLessonsHubListOptsForSnapshot(args: {
  ...
  const taxNorm =
    ap && args.alliedTaxonomyFilter?.trim()
      ...
      : null;
  const allied: Pick<MarketingHubLessonsListOptions, "alliedProfessionKey"> = ap ? { alliedProfessionKey: ap } : {};
  const tax: Pick<MarketingHubLessonsListOptions, "taxonomySlugsIn"> = taxNorm ? { taxonomySlugsIn: [taxNorm] } : {};
