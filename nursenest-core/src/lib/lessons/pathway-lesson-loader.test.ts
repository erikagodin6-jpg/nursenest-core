...
describe("pathway-lesson-loader normalization", () => {
  it("returns five ordered sections for a known catalog lesson", () => {
    const lesson = getPathwayLesson("ca-rn-nclex-rn", "fluid-balance-acute-care");
    assert.ok(lesson);
  ...
  it("returns undefined for unknown slug", () => {
    assert.equal(getPathwayLesson("ca-rn-nclex-rn", "does-not-exist-zzz"), undefined);
  });
});
