...
function ensureNonEmptyHubResult(
  ...
  if (lessons.length > 0 && result.kept.length === 0) {
    safeServerLog("pathway_lessons", "hub_verify_zero_kept_bypass_returning_prepared", {
      pathway_id: pathwayId,
      prepared_count: String(lessons.length),
      verify_kept_count: "0",
      outcome: "returned_prepared_degraded_rows",
    });

    return {
      kept: lessons
        .filter((l) => pathwayLessonHasRenderableHubSlug(l))
        .map((l) => ({
          ...l,
          hubMarketingDegraded: true,
          hubMarketingDegradedReason: "partial_content",
        })),
      excluded: result.excluded,
      diagnostics: {
        ...result.diagnostics,
        keptRowCount: lessons.filter((l) => pathwayLessonHasRenderableHubSlug(l)).length,
        degradedHubRowCount: lessons.filter((l) => pathwayLessonHasRenderableHubSlug(l)).length,
        droppedRowCount: 0,
      },
    };
  }
...
export async function verifyMarketingHubLessonRowsResolve(
  ...
  const exclusionReasonsRanked: Array<{ reason: HubMarketingLessonDetailFailureReason; count: number }> = [
    ...
    .map(([reason, count]) => ({
      reason: reason as HubMarketingLessonDetailFailureReason,
      count: count ?? 0,
    }))
    .filter((x) => x.count > 0)
  ...
  if (lessons.length > 0 && kept.length === 0) {
    safeServerLog("pathway_lessons", "hub_verify_pipeline_zero_kept", {
      ...
      unique_slugs: String(uniqueSlugs.length),
      reasons_json: JSON.stringify(excludedByReason),
      outcome: "pipeline_break_bypassed",
    });
  }
  ...
  if (lessons.length > 0 && kept.length === 0) {
    safeServerLog("pathway_lessons", "hub_verify_prepared_positive_zero_kept_critical", {
      ...
      outcome: options?.skipZeroKeptPipelineInvariant ? "all_rows_excluded_tests" : "all_rows_excluded_bypassed",
    });
  }
