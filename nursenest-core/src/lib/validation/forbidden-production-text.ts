/**
 * Central forbidden visible-copy detection for marketing/public surfaces.
 * Used by Playwright DOM tests, Node HTTP smoke scripts, and unit tests — keep matchers aligned.
 */
...
/** Serialized React / bad runtime values that must never appear as visible text. */
export const FORBIDDEN_RUNTIME_SUBSTRINGS = [
  "undefined",
  "null",
  "[object object]",
  "missing key",
...
export const FORBIDDEN_SHOUTY_DESIGN_TOKEN_RE = new RegExp(
  String.raw`\b(?:KICKER|LABEL|TITLE|DESCRIPTION|LEAD|SUBTITLE|CTA|BUTTON|HEADING|EYEBROW|INTRO|STUB|TODO|TBD|PLACEHOLDER|LINK|BODY)\b`,
);
...
export type MarketingDomPlaceholderViolation =
  ...
  | { kind: "runtime_substring"; needle: string };

/** @deprecated Use {@link collectForbiddenProductionTextViolations} — alias kept for Playwright imports. */
export function collectMarketingDomPlaceholderViolations(visibleBodyInnerText: string): MarketingDomPlaceholderViolation[] {
  return collectForbiddenProductionTextViolations(visibleBodyInnerText);
}

export function collectForbiddenProductionTextViolations(visibleBodyInnerText: string): MarketingDomPlaceholderViolation[] {
  ...
  for (const needle of FORBIDDEN_RUNTIME_SUBSTRINGS) {
    if (lowerCollapsed.includes(needle)) {
      out.push({ kind: "runtime_substring", needle });
    }
  }
...
export function formatMarketingDomViolationMessage(
  ...
  const parts = violations.slice(0, 12).map((v) => {
    switch (v.kind) {
      ...
      case "runtime_substring":
        return `runtime_substring:${JSON.stringify(v.needle)}`;
      default:
        return String(v);
    }
  });
  const suffix = violations.length > 12 ? ` (+${violations.length - 12} more)` : "";
  return `[forbidden-production-text] ${route}: ${parts.join(" | ")}${suffix}`;
}

/**
 * Strip script/style/noscript and tags for HTTP-level HTML probes (no cheerio).
 */
export function htmlToProbePlainText(html: string): string {
  ...
  s = s.replace(/&#\d+;/g, " ");
  s = s.replace(/&[a-z]+;/gi, " ");
  return normalizeMarketingDomText(s);
}
