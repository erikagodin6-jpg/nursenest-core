...
export async function buildContentScalabilityReport(): Promise<ContentScalabilityReport> {
  ...
  if (cachedScalabilityReport && nowTs - cachedScalabilityReport.at < REPORT_CACHE_MS) {
    return cachedScalabilityReport.value;
  }
  if (scalabilityInFlight) return scalabilityInFlight;
  ...
  scalabilityInFlight = (async () => {
      ...
      databaseConfigured
        ? await withDatabaseFallbackTimeout(
            async () => {
              ...
              const byStatus = await prisma.examQuestion.groupBy({
                by: ["status"],
                _count: { _all: true },
              });
    ...
    cachedScalabilityReport = { at: Date.now(), value: report };
    return report;
  })().finally(() => {
    scalabilityInFlight = null;
  });

  return scalabilityInFlight;
}
