/**
 * Hard keyword lists per taxonomy leaf. Matching is deterministic (substring / word checks in classifier).
 */

export const KEYWORD_MAP: Record<string, readonly string[]> = {
  cardiovascular: [
    "heart",
    "cardiac",
    "mi",
    "myocardial",
    "arrhythmia",
    "hypertension",
    "angina",
    "stemi",
    "nstemi",
    "heart failure",
    "cardiogenic",
    "pericard",
    "endocard",
    "ecg",
    "ekg",
  ],
  respiratory: [
    "lung",
    "copd",
    "asthma",
    "oxygen",
    "ventilation",
    "pneumonia",
    "pulmonary",
    "airway",
    "abg",
    "acid-base",
    "acid base",
    "arterial blood gas",
  ],
  neurological: [
    "brain",
    "stroke",
    "seizure",
    "neuro",
    "cns",
    "icp",
    "mening",
    "parkinson",
    "delirium",
    "tbi",
    "psychiat",
    "mental health",
    "depression",
    "anxiety",
    "suicide",
    "bipolar",
    "schizo",
    "ptsd",
  ],
  endocrine: ["diabetes", "insulin", "thyroid", "dka", "hhs", "adrenal", "pituitary", "cortisol", "glucose"],
  renal_genitourinary: [
    "kidney",
    "renal",
    "urinary",
    "dialysis",
    "nephro",
    "creatinine",
    "uti",
    "bladder",
    "aki",
    "ckd",
    "kidney transplant",
    "transplant",
    "graft",
    "immunosuppression",
  ],
  gastrointestinal: ["liver", "bowel", "gi", "pancreas", "hepat", "intestinal", "crohn", "colitis", "cirrhosis"],
  hematology_oncology: [
    "anemia",
    "cancer",
    "leukemia",
    "lymphoma",
    "chemo",
    "oncolog",
    "sickle",
    "thrombocyt",
    "hematologic",
    "hematology",
    "neutropenic",
    "neutropenia",
    "transfusion",
  ],
  musculoskeletal: ["anatomy", "physiology", "bone", "fracture", "joint", "orthop", "arthritis", "spine", "rheumat", "mobility"],
  integumentary: ["skin", "wound", "ulcer", "dermat", "rash", "pressure injury", "cellulitis", "burn"],
  immune_infectious: [
    "infection",
    "sepsis",
    "immune",
    "antibiotic",
    "hiv",
    "tuberculosis",
    "septic",
    "antimicrobial",
    "hand hygiene",
    "standard precautions",
    "infection control",
    "isolation",
    "ppe",
    "immunization",
    "vaccine",
    "vaccination",
  ],
  reproductive_obstetrics: [
    "pregnancy",
    "labor",
    "uterus",
    "obstetric",
    "postpartum",
    "fetal",
    "neonatal",
    "lactation",
    "fhr",
    "deceleration",
    "preeclampsia",
    "eclampsia",
    "rh incompatibility",
  ],
  pediatrics: [
    "infant",
    "child",
    "pediatric",
    "adolescent",
    "newborn",
    "neonate",
    "growth failure",
    "failure to thrive",
    "non-accidental trauma",
  ],

  ethics: ["ethics", "ethical", "moral", "bioethics", "advocacy", "ethical distress"],
  legal_regulation: ["law", "legal", "regulation", "hipaa", "consent", "malpractice", "jurisprudence", "nurse practice act"],
  documentation: ["charting", "documentation", "record", "ehr", "emar"],
  communication: ["communication", "therapeutic", "handoff", "sbar", "interprofessional", "care conference"],
  scope_of_practice: ["scope", "responsibility", "nurse practice act"],
  delegation_supervision: ["delegate", "delegation", "assignment", "supervision"],
  leadership_management: ["leadership", "management"],
  patient_safety_quality: [
    "safety",
    "quality",
    "quality improvement",
    "qi",
    "incident reporting",
    "risk",
    "root cause",
    "near miss",
    "falls",
    "fall prevention",
    "hourly rounding",
    "restraint",
    "rapid response",
  ],

  cardiovascular_drugs: [
    "beta blocker",
    "ace inhibitor",
    "arb",
    "statin",
    "anticoag",
    "digoxin",
    "nitroglycerin",
    "antihypertensive",
  ],
  cns_drugs: ["benzodiazep", "opioid", "antidepress", "antipsych", "anticonvuls"],
  endocrine_drugs: ["metformin", "sulfonylurea", "glp-1", "insulin lispro", "levothyroxine"],
  anti_infectives: ["vancomycin", "penicillin", "cephalosporin", "macrolide", "antifungal"],
  pain_sedation: ["medication", "medications", "analgesic", "sedation", "anxiolytic", "nsaid", "acetaminophen"],

  test_taking: ["test taking", "elimination", "priority question", "nclex strategy"],
  study_strategy: ["study plan", "pomodoro", "spaced repetition", "active recall"],
} as const satisfies Record<string, readonly string[]>;
