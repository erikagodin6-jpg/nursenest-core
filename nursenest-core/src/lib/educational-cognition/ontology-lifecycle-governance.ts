...
function applyAliasesToSnapshot(
  ...
  const reasoningPatterns = (snapshot.reasoningPatterns ?? []).map((p) =>
    (focusRenames[p] ?? aliases[p] ?? p) as ClinicalJudgmentPattern,
  );
  return { ...snapshot, competencyStates, reasoningPatterns };
}
