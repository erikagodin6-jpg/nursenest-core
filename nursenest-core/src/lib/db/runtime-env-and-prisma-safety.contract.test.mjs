    ...
    withCleanDbEnv(() =>
      withTempEnv({}, (envRoot) => {
        ...
        try {
          ...
          assert.equal(result.installTimeGenerate, true);
          assert.match(process.env.DATABASE_URL, /^postgresql:\/\/prisma:prisma@127\.0\.0\.1/);
          assert.equal(process.env.DIRECT_URL, process.env.DATABASE_URL);
          assert.equal(
            logs.some((line) => line.includes("Install-time Prisma generate detected; using local placeholder DATABASE_URL for code generation only.")),
            true,
          );
        } finally {
