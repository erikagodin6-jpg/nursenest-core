  ...
  it("allows DigitalOcean build Prisma generate with DATABASE_URL only", () =>
    withCleanDbEnv(() =>
      withTempEnv({}, (envRoot) => {
        ...
        assert.equal(logs.some((line) => line.includes("Build-time Prisma generate detected; runtime database validation skipped.")), true);
      }),
    ));

  it("allows GitHub Actions install-time Prisma generate without database secrets", () =>
    withCleanDbEnv(() =>
      withTempEnv({}, (envRoot) => {
        ...
        assert.equal(process.env.DATABASE_URL, "postgresql://prisma:prisma@127.0.0.1:65432/nn_prisma_codegen?schema=public");
        assert.equal(process.env.DIRECT_URL, process.env.DATABASE_URL);
        assert.equal(logs.some((line) => line.includes("Install-time Prisma generate detected; DATABASE_URL/DIRECT_URL validation skipped.")), true);
      }),
    ));
