...
export const MARKETING_PRIMARY_CTA_CLASS =
  "nn-btn-primary inline-flex min-h-[48px] min-w-0 w-full flex-wrap items-center justify-center gap-x-2 gap-y-1 px-8 py-3 text-center text-base font-semibold text-balance break-words transition-[filter] hover:bg-role-cta-hover sm:min-h-[52px] sm:w-auto sm:max-w-[min(100%,24rem)] sm:px-10 sm:text-lg" as const;
...
export const MARKETING_SECONDARY_CTA_CLASS =
  "nn-btn-secondary inline-flex min-h-[48px] min-w-0 w-full flex-wrap items-center justify-center gap-x-2 gap-y-1 px-6 py-3 text-center text-sm font-semibold text-balance break-words sm:min-h-[52px] sm:w-auto sm:max-w-[min(100%,22rem)] sm:px-7 sm:text-base" as const;
