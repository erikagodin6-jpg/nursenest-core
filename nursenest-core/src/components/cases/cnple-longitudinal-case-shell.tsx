...
import type { CaseStepPayload, CaseStepAdvanceResult, PatientCase } from "@/lib/cases/longitudinal-case-types";
import {
  PatientSummaryPanel,
  ClinicalTimeline,
...
import type { MedicationEntry, LabResult, TimelineEvent } from "@/components/clinical/clinical-case-ui";
...
export function CnpleLongitudinalCaseShell({
  ...
  const labs = buildLabResults(step.diagnosticArtifacts ?? []);
  ...
  return (
    <div className="cnple-case-shell w-full" data-cnple-case="shell">
      ...
      <div className="mt-4 flex gap-5 lg:grid lg:grid-cols-[17rem_1fr_19rem] lg:items-start lg:gap-6">
        ...
        >
          {labs.length > 0 && <LabTrendTable results={labs} />}
