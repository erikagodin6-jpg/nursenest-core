...
async function safeLoadMarketingMessages(locale: string) {
  try {
    return await loadMarketingMessages(locale);
  } catch {
    return await loadMarketingMessages(DEFAULT_MARKETING_LOCALE);
  }
}

function normalizeMarketingRegion(region: unknown): MarketingRegionToggle {
...
export async function PublicLessonsHubView({ locale }: { locale: string }) {
  const marketingRegion = normalizeMarketingRegion(await getMarketingRegionFromCookies());

  const [m, en] = await Promise.all([
    safeLoadMarketingMessages(locale),
    safeLoadMarketingMessages(DEFAULT_MARKETING_LOCALE),
  ]);
  ...
  return (
    <>
      ...
      <MarketingPublicStudyLanding
        ...
        signupCta={{
          href: signupLessons,
          label: t("pages.publicLessons.ctaCreateAccount"),
        }}
      >
