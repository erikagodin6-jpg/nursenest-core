 ...
 */
export function PricingLearnerFaq() {
  const { t } = useMarketingI18n();

  return (
    <section
      aria-labelledby="pricing-learner-faq-heading"
      className="scroll-mt-20 border-y border-[color-mix(in_srgb,var(--semantic-chart-4)_24%,var(--semantic-border-soft))] bg-[color-mix(in_srgb,var(--semantic-chart-4)_07%,var(--semantic-surface))] py-10 md:py-12"
      data-testid="section-pricing-learner-faq"
    >
      <div className="mx-auto max-w-3xl px-4 sm:px-0">
        <FadeUp whenInView once viewMargin="-28px" className="mb-6 text-center">
          <div className="mx-auto mb-3 flex justify-center">
            <span
              className="inline-flex items-center gap-2 rounded-full border border-[color-mix(in_srgb,var(--semantic-chart-3)_26%,var(--semantic-border-soft))] bg-[color-mix(in_srgb,var(--semantic-chart-3)_10%,var(--semantic-surface))] px-4 py-1.5 text-xs font-semibold uppercase tracking-wide text-[var(--semantic-chart-3)]"
            >
              <HeartHandshake className="h-3.5 w-3.5" aria-hidden />
              {t("pages.pricing.learnerFaq.eyebrow")}
            </span>
          </div>
          <h2 id="pricing-learner-faq-heading" className="nn-marketing-h2 text-balance text-[var(--semantic-text-primary)]">
            {t("pages.pricing.learnerFaq.heading")}
          </h2>
          <p className="nn-marketing-body-sm mx-auto mt-2 max-w-xl text-[var(--semantic-text-secondary)]">
            {t("pages.pricing.learnerFaq.subheading")}
          </p>
        </FadeUp>
            ...
            >
              <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-left marker:content-none [&::-webkit-details-marker]:hidden">
                <span className="text-sm font-semibold text-[var(--semantic-text-primary)]">
                  {t(`pages.pricing.learnerFaq.${key}Question`)}
                </span>
                <ChevronDown
              ...
              <p className="nn-marketing-body-sm mt-3 border-t border-[var(--semantic-border-soft)] pt-3 text-[var(--semantic-text-secondary)]">
                {t(`pages.pricing.learnerFaq.${key}Answer`)}
              </p>
            </details>
          ))}
        </div>
