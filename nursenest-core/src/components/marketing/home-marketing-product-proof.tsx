...
export function HomeMarketingProductProof({ questionCount, lessonCount }: Props) {
  ...
  const q =
    questionCount != null && questionCount > 0 ? questionCount.toLocaleString(locale.replace(/_/g, "-")) : "—";
  const l = lessonCount != null && lessonCount > 0 ? lessonCount.toLocaleString(locale.replace(/_/g, "-")) : "—";

  return (
    <div className="space-y-4" data-testid="home-product-proof-wrap">
      <div className="max-w-2xl">
        <h2 className="text-lg font-bold text-[var(--theme-heading-text)] sm:text-xl">{t("home.productProof.title")}</h2>
        <p className="mt-2 text-sm text-[var(--theme-muted-text)]">{t("home.productProof.sub")}</p>
          ...
          <div className="rounded-2xl border border-[var(--theme-card-border)] bg-card p-4 shadow-[var(--shadow-card)] sm:p-5">
            ...
            <dl className="mt-4 grid gap-2 text-sm">
              ...
              <div className="flex justify-between gap-4">
                ...
                <dd className="font-medium text-emerald-700 dark:text-emerald-300">{t("home.productProof.trendExample")}</dd>
              </div>
            </dl>
