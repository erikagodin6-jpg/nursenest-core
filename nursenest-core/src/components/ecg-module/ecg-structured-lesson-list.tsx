...
export function EcgStructuredLessonList({ level }: { level: EcgLevel }) {
  ...
  return (
    <section className="space-y-10" aria-label={`${level} structured ECG lessons`}>
      <div className="rounded-2xl border border-[color-mix(in_srgb,var(--semantic-info)_22%,var(--semantic-border-soft))] bg-[color-mix(in_srgb,var(--semantic-info)_04%,var(--semantic-surface))] p-7 shadow-[var(--semantic-shadow-soft)] sm:p-9">
      ...
      {lessons.map((lesson, lessonIndex) => {
        ...
        return (
          ...
          >
            <header className="border-b border-[var(--semantic-border-soft)] bg-[color-mix(in_srgb,var(--semantic-panel-cool)_42%,var(--semantic-surface))] p-6 sm:p-8">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <h3 className="mt-2 text-2xl font-bold tracking-tight text-[var(--semantic-text-primary)]">
                    {lesson.title}
                  </h3>
                </div>
              ...
              <p className="mt-4 max-w-3xl text-sm leading-relaxed text-[var(--semantic-text-secondary)]">
                {lesson.summary}
              </p>
            </header>
            ...
            <div className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
              ...
              <div className="space-y-5">
                <section className="rounded-xl border border-[var(--semantic-border-soft)] bg-[var(--semantic-surface)] p-5">
                  ...
                  <p className="mt-3 text-sm leading-relaxed text-[var(--semantic-text-secondary)]">{lesson.mechanism}</p>
                </section>
                {lesson.pWaveTeaching ? (
                  <section className="rounded-xl border border-[var(--semantic-border-soft)] bg-[var(--semantic-surface)] p-5">
                    <h4 className="text-sm font-semibold text-[var(--semantic-text-primary)]">P-wave teaching</h4>
                    <p className="mt-3 text-sm leading-relaxed text-[var(--semantic-text-secondary)]">{lesson.pWaveTeaching}</p>
                  </section>
                ) : null}
                {lesson.prTeaching ? (
                  <section className="rounded-xl border border-[var(--semantic-border-soft)] bg-[var(--semantic-surface)] p-5">
                    <h4 className="text-sm font-semibold text-[var(--semantic-text-primary)]">PR interval teaching</h4>
                    <p className="mt-3 text-sm leading-relaxed text-[var(--semantic-text-secondary)]">{lesson.prTeaching}</p>
                  </section>
                ) : null}
                {lesson.qrsTeaching ? (
                  <section className="rounded-xl border border-[var(--semantic-border-soft)] bg-[var(--semantic-surface)] p-5">
                    <h4 className="text-sm font-semibold text-[var(--semantic-text-primary)]">QRS teaching</h4>
                    <p className="mt-3 text-sm leading-relaxed text-[var(--semantic-text-secondary)]">{lesson.qrsTeaching}</p>
                  </section>
                ) : null}
              </div>
            </div>

            <div className="grid gap-6 border-t border-[var(--semantic-border-soft)] p-6 sm:p-8 lg:grid-cols-3">
            ...
            <div className="border-t border-[var(--semantic-border-soft)] bg-[color-mix(in_srgb,var(--semantic-warning)_04%,var(--semantic-surface))] p-6 sm:p-8">
              <SectionList title="Differential reasoning — what rules out the look-alikes" items={lesson.differentiators} />
            </div>

            <div className="space-y-6 border-t border-[var(--semantic-border-soft)] p-6 sm:p-8">
