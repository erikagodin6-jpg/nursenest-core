    ...
    return (
      <span
        ...
        style={{
          ...
          color:
            "color-mix(in srgb, var(--semantic-warning) 82%, var(--semantic-text-primary))",
          border:
...
export function LessonPageHeader({
  ...
  const audience =
    audienceTiers && audienceTiers.length > 0
      ? audienceTiers
          .map((t) => AUDIENCE_LABEL[t] ?? t.toUpperCase())
          .join(" · ")
      : null;
    ...
    <header className="nn-lesson-page-header nn-premium-learner-lesson-hero">
      <div className="flex flex-col gap-5">
        <div className="min-w-0 flex-1">
          ...
          >
            {[audience, pathwayLine, bodyLabel ?? topic]
              .filter(Boolean)
          ...
          <div className="nn-lesson-hero-meta mt-6 flex flex-wrap items-center gap-2 text-[var(--semantic-text-muted)]">
            ...
            {bodyLabel ? (
              <span className="nn-lesson-hero-chip capitalize">
                {bodyLabel}
              </span>
            ) : null}
            {audience ? (
              <span className="nn-lesson-hero-chip">{audience}</span>
            ) : null}
            <span className="nn-lesson-hero-chip tabular-nums">
              {sectionCount} section{sectionCount !== 1 ? "s" : ""}
            </span>
            {assessmentHint ? (
              <span className="nn-lesson-hero-chip font-medium text-[var(--semantic-info)]">
                {assessmentHint}
              </span>
            ) : null}
            <span
              className="nn-lesson-hero-progress"
              aria-hidden="true"
