...
export function PathwayLessonFigures({ figures }: { figures: PathwayLessonFigure[] }) {
  ...
  const visible = usable.filter((f) => !hiddenIds[f.id]);
  ...
  return (
    <div className="mt-5 space-y-5">
      {visible.map((f) => (
        <figure
          key={f.id}
          className="overflow-hidden rounded-2xl border border-border bg-[var(--theme-muted-surface)]/40 shadow-sm"
        >
          <SafeLessonRemoteImage
            src={f.url}
            alt={f.alt?.trim() ? f.alt : "Lesson illustration"}
            className="max-h-[min(65vh,480px)] h-auto w-full max-w-full object-contain p-2"
            onHidden={() => hide(f.id)}
          />
          {f.caption || f.kind ? (
            <figcaption className="border-t border-border px-4 py-2.5 text-[0.8125rem] leading-relaxed text-muted-foreground">
              ...
              {f.kind ? (
                <span
                  className={`${f.caption ? "ml-2 " : ""}rounded-md bg-muted px-1.5 py-0.5 font-semibold text-[0.6875rem] uppercase tracking-wide text-muted-foreground`}
                >
                  {f.kind.replace(/_/g, " ")}
                </span>
              ) : null}
