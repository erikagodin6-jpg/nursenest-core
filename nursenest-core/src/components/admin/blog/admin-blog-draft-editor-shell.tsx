...
/**
 * Premium draft editor chrome: section jump links + optional live HTML preview column.
 * Admin-only; used by the blog control panel after generation.
 */
    ...
    <div className="flex flex-col gap-8 xl:flex-row xl:items-start">
      <aside className="xl:sticky xl:top-20 xl:z-10 xl:w-56 xl:shrink-0">
        <div className="rounded-2xl border border-border/80 bg-[var(--theme-card-bg)]/95 p-4 shadow-sm backdrop-blur-sm">
          <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Draft sections</p>
          <nav className="mt-3 flex max-h-[min(70vh,32rem)] flex-col gap-0.5 overflow-y-auto pr-1 text-sm" aria-label="Jump to draft section">
            {navItems.map((item) => (
              <a
                key={item.id}
                href={`#${item.id}`}
      ...
      {previewOpen ? (
        <aside className="xl:sticky xl:top-20 xl:z-10 xl:w-[min(100%,440px)] xl:shrink-0">
          <div className="rounded-2xl border border-border/80 bg-muted/20 p-1 shadow-inner">
            <div className="max-h-[min(85vh,56rem)] overflow-y-auto rounded-[14px] bg-[var(--theme-page-bg)] p-4">{previewPanel}</div>
          </div>
        </aside>
      ) : null}
    </div>
...
/** Consistent section card with anchor id for in-page nav. */
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border/50 pb-3">
        <div>
          <h3 className="text-sm font-semibold tracking-tight text-[var(--theme-heading-text)]">{title}</h3>
          {description ? <p className="mt-1 max-w-prose text-[11px] text-muted-foreground">{description}</p> : null}
        </div>
