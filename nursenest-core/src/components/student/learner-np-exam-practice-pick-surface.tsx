      ...
      {pathways.length === 0 ? (
        ...
        <ul className="grid gap-4 sm:grid-cols-2">
          {pathways.map((p) => {
            ...
            const actions = isCnple
              ? [
                  {
                    href: "/app/practice-tests?pathwayId=ca-np-cnple",
                    label: "Practice Tests",
                    accent: "hover:border-[var(--semantic-info)]",
                  },
                  {
                    href: "/app/cases/cnple",
                    label: "LOFT Simulation",
                    accent: "hover:border-[var(--semantic-warning)]",
                  },
                  {
                    href: "/app/flashcards?pathwayId=ca-np-cnple",
                    label: "Flashcards",
                    accent: "hover:border-[var(--semantic-chart-2)]",
                  },
                  {
                    href: "/app/lessons?pathwayId=ca-np-cnple",
                    label: "Lessons",
                    accent: "hover:border-[var(--semantic-success)]",
                  },
                  {
                    href: questionsHref,
                    label: "Question bank",
                    accent: "hover:border-[var(--semantic-brand)]",
                  },
                ]
              : [
                  {
                    href: `/app/practice-tests?pathwayId=${encodeURIComponent(p.id)}`,
                    label: "Practice exams & CAT",
                    accent: "hover:border-[var(--semantic-info)]",
                  },
                  {
                    href: questionsHref,
            ...
            return (
              ...
              >
                ...
                <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
                  {actions.map((action) => (
                    <Link
                      key={`${p.id}-${action.label}`}
                      href={action.href}
                      className={`inline-flex items-center justify-center rounded-lg border border-[var(--semantic-border-soft)] bg-[var(--theme-card-bg)] px-3 py-2 text-sm font-medium text-[var(--semantic-text-primary)] transition-colors ${action.accent}`}
                    >
                      {action.label}
                    </Link>
                  ))}
