 ...
 * Its practice tile exposes learner-app links instead of sending signed-in learners to public marketing pages.
 */
      ...
      {pathways.length === 0 ? (
        ...
        <ul className="grid gap-4 sm:grid-cols-2">
          {pathways.map((p) => {
            ...
            const simulationHref = "/app/cases/cnple";
            const flashcardsHref = `/app/flashcards?pathwayId=${encodeURIComponent(p.id)}`;
            const lessonsHref = `/app/lessons?pathwayId=${encodeURIComponent(p.id)}`;
            const questionsHref = pathwayHubAppQuestionsHref(p.id);
            const linkClass =
              "inline-flex items-center justify-center rounded-lg border border-[var(--semantic-border-soft)] bg-[var(--theme-card-bg)] px-3 py-2 text-sm font-medium text-[var(--semantic-text-primary)] transition-colors hover:border-[var(--semantic-info)]";
                ...
                <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
                  {isCnple ? (
                    <>
                      ...
                      <Link href={simulationHref} className={linkClass}>
                        LOFT Simulation
                      </Link>
                      <Link href={flashcardsHref} className={linkClass}>
                        Flashcards
                      </Link>
                      <Link href={lessonsHref} className={linkClass}>
                        Lessons
                      </Link>
                    </>
                  ) : (
