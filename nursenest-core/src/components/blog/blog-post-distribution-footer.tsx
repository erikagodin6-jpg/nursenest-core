...
export function BlogPostDistributionFooter({ exam, relatedLessonPaths, relatedQuestionIds, relatedTools }: Props) {
  ...
  return (
    <aside className="mt-6 space-y-6 border-t border-[var(--theme-separator)] pt-6">
      ...
      {!hasQuestionIds && lessons.length === 0 ? null : (
        <div className="rounded-xl border border-[var(--theme-card-border)] bg-[var(--theme-muted-surface)]/50 p-4 text-sm text-[var(--theme-body-text)]">
          ...
          <p className="mt-1 text-muted-foreground">
            {hasQuestionIds
              ? "Use the question bank with an account to work items matched to this topic. Rationales stay in the app after you sign in."
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Link href="/app/study-plan" className="text-sm font-semibold text-primary underline-offset-4 hover:underline">
          Study plan
        </Link>
      </div>
    </aside>
