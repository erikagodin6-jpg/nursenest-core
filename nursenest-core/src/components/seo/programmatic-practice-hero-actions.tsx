...
export function ProgrammaticPracticeHeroActions({ locale, slug }: { locale: string; slug: string }) {
  ...
  return (
    <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
      ...
      >
        {t("programmatic.practiceHero.startQuestions")}
      ...
      >
        {t("programmatic.practiceHero.studyLessons")}
      </Link>
      <Link
      ...
      >
        {t("programmatic.practiceHero.openTestBank")}
      </Link>
      <Link
        href={hub.ctas.exams}
      ...
      >
        {t("programmatic.practiceHero.takeTest")}
      </Link>
      {hub.ctas.studyPlan ? (
      ...
      <p className="w-full text-xs font-medium text-muted-foreground sm:pl-1">{t("programmatic.practiceHero.footerLine")}</p>
    </div>
  );
}
