...
import { listVerifiedStudyDecksSharedWithUser } from "@/lib/verified-study/verified-study-decks.server";

type PageProps = { searchParams: Promise<{ pathwayId?: string | string[] }> };

export async function generateMetadata(): Promise<Metadata> {
  return buildStudyToolsActivityMetadata("Shared study decks");
}
...
export default async function VerifiedStudySharedDecksPage({ searchParams }: PageProps) {
  const pathwayId = await pathwayIdFromSearch(searchParams);
  const session = await getProtectedRouteSession("(student).app.(learner).study-tools.shared");
  const userId = (session?.user as { id?: string })?.id ?? "";

  const decks =
    userId && pathwayId ? await listVerifiedStudyDecksSharedWithUser({ userId, pathwayId }) : [];

  return (
    <StudyToolsActivityShell
      title="Shared with you"
      description="Decks another learner invited you to view for this pathway."
      pathwayId={pathwayId}
    >
      {!pathwayId ? (
        <p className="text-sm text-[var(--semantic-warning)]">Add pathwayId to the query string.</p>
      ) : decks.length === 0 ? (
        <p className="text-sm text-[var(--theme-body-text)]">No shared decks for this pathway.</p>
      ) : (
          ...
          {decks.map((d) => (
            <li key={d.id}>
              <Link
                ...
                className="block rounded-lg border border-[var(--semantic-border-soft)] bg-[color-mix(in_srgb,var(--semantic-panel-warm)_12%,var(--bg-card))] px-3 py-2 text-sm font-medium text-[var(--semantic-text-primary)]"
              >
                {d.title}{" "}
