...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = merged["exams.australia.metaTitle"] ?? merged["exams.australia.title"] ?? "Australia nursing registration";
  const description =
    merged["exams.australia.metaDescription"] ??
    merged["exams.australia.lead"] ??
    "AHPRA, OSCE, OBA, and international nurse pathways.";
  ...
  return safeGenerateMetadata(
    ...
    },
    {
      pathname: PATH,
      locale,
      routeGroup: "marketing.locale.examsAustralia",
    },
  );
}

export default async function AustraliaExamsHubLocalePage({ params }: Props) {
  const { locale } = await params;
  if (!isCoreHostedNonDefaultLocale(locale)) notFound();
  ...
  return <ExamsAustraliaHubShell locale={locale} messages={merged} />;
}
