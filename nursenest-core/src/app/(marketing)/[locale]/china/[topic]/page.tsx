...
import {
  CHINA_TOPIC_SLUGS,
  ChinaTopicView,
  parseChinaTopicParam,
} from "@/components/marketing/exams-china/china-topic-view";
import {
...
export function generateStaticParams() {
  ...
  for (const loc of CORE_HOSTED_MARKETING_LOCALES) {
    for (const topic of CHINA_TOPIC_SLUGS) {
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title =
    merged[`exams.china.subpage.${slug}.metaTitle`] ??
    merged[`exams.china.subpage.${slug}.title`] ??
    "China nursing";
  const description =
    merged[`exams.china.subpage.${slug}.metaDescription`] ??
    merged[`exams.china.subpage.${slug}.title`] ??
  ...
  return safeGenerateMetadata(
    ...
    },
    {
      pathname: path,
      locale,
      routeGroup: "marketing.locale.chinaTopic",
    },
  );
}

export default async function ChinaTopicLocalePage({ params }: Props) {
  const { locale, topic } = await params;
  if (!isCoreHostedNonDefaultLocale(locale)) notFound();
  const slug = parseChinaTopicParam(topic);
  if (!slug) notFound();
