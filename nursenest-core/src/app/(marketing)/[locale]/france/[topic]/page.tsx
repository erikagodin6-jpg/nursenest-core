...
import {
  FRANCE_TOPIC_SLUGS,
  FranceTopicView,
  parseFranceTopicParam,
} from "@/components/marketing/exams-france/france-topic-view";
import {
...
export function generateStaticParams() {
  ...
  for (const loc of CORE_HOSTED_MARKETING_LOCALES) {
    for (const topic of FRANCE_TOPIC_SLUGS) {
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title =
    merged[`exams.france.subpage.${slug}.metaTitle`] ??
    merged[`exams.france.subpage.${slug}.title`] ??
    "France nursing";
  const description =
    merged[`exams.france.subpage.${slug}.metaDescription`] ??
    merged[`exams.france.subpage.${slug}.title`] ??
    "France nursing registration.";
  ...
  return safeGenerateMetadata(
    ...
    },
    {
      pathname: path,
      locale,
      routeGroup: "marketing.locale.franceTopic",
    },
  );
}

export default async function FranceTopicLocalePage({ params }: Props) {
  const { locale, topic } = await params;
  if (!isCoreHostedNonDefaultLocale(locale)) notFound();
  const slug = parseFranceTopicParam(topic);
  if (!slug) notFound();
