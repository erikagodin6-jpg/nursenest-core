...
import { nursingScopedCareerLabels } from "@/lib/blog/blog-scoped-career-hubs";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!visible || !meta) {
    return {
      ...
      alternates: { canonical: absoluteUrl("/blog/rn") },
    };
  }
  return safeGenerateMetadata(
    async () => {
      const auto = generateBlogSEOFromPostRow(
        ...
        { canonicalPath: `/blog/rn/${postSlug}` },
      );
      ...
      const canonical = absoluteUrl(`/blog/rn/${postSlug}`);
    ...
    { pathname: `/blog/rn/${postSlug}`, routeGroup: "marketing.default.blog.rn.post" },
  );
}

export default async function BlogRnArticlePage({ params }: Props) {
  const { slug: postSlug } = await params;
  const labels = nursingScopedCareerLabels(CAREER);
  const locale = await getMarketingLocaleForDefaultRoute();
  ...
  const auto = generateBlogSEOFromPostRow(
    ...
    { canonicalPath: `/blog/rn/${post.slug}` },
  );
  ...
  const crumbs = [
    { name: "Blog", href: "/blog" },
    { name: `${labels.short} articles`, href: "/blog/rn" },
    { name: h1Text },
  ];
  const schemaItems = [
    { name: "Blog", item: absoluteUrl("/blog") },
    { name: `${labels.short} articles`, item: absoluteUrl("/blog/rn") },
    { name: h1Text, item: absoluteUrl(`/blog/rn/${post.slug}`) },
  ];
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-12">
      ...
      <Link href="/blog/rn" className="text-sm font-medium text-primary hover:underline">
        ← Back to RN articles
      </Link>
      <BlogStudyAnchorStrip
        ...
        className="mt-6"
        labelledById="rn-blog-study-top"
      />
      ...
      {emitFaqJsonLd ? (
        <section
          ...
          aria-labelledby="rn-blog-faq-heading"
        >
          <h2
            id="rn-blog-faq-heading"
      ...
      <BlogStudyAnchorStrip
        ...
        flashcardsAnchorText={studyAnchorsText.flashcards}
        className="mt-10"
        labelledById="rn-blog-study-bottom"
      />
    </article>
  );
}
