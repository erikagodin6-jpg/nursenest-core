...
export function generateStaticParams() {
  return listAuthorityClusterPages()
    .filter((page) => page.cluster === "respiratory-therapy" && page.slug !== "overview")
    .map((page) => ({ topic: page.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { topic } = await params;
  const page = getAuthorityClusterPage("respiratory-therapy", topic);
  if (!page) return {};
  return {
    title: page.title,
...
export default async function RespiratoryTherapyAuthorityTopicPage({ params }: Props) {
  const { topic } = await params;
  const page = getAuthorityClusterPage("respiratory-therapy", topic);
