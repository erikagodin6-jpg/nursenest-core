...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const path = newGradWorkAreaHubPath("canada", workArea);
  return safeGenerateMetadata(
    async () => {
      if (!def) return { title: "Not found" };
      return {
        title: `${def.title} | New Grad | Canada | NurseNest`,
    ...
    { pathname: path, routeGroup: "marketing.default.canada.new_grad.work_area" },
  );
}

export default async function CanadaNewGradWorkAreaPage({ params }: Props) {
  const { workArea } = await params;
  const def = getNewGradWorkAreaBySlug(workArea);
  if (!def) notFound();
  ...
  const hubPath = newGradWorkAreaHubPath("canada", workArea);
  ...
  const crumbs = [
    ...
    { name: "New Grad", href: CANADA_NEW_GRAD_MARKETING_HUB_PATH },
    { name: def.title, href: undefined },
  ];
  const schemaItems = [
    { name: "Home", item: absoluteUrl("/") },
    { name: "Canada", item: absoluteUrl("/canada") },
    { name: "New Grad", item: absoluteUrl(CANADA_NEW_GRAD_MARKETING_HUB_PATH) },
  ...
  return (
    <div className="nn-marketing-surface mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
      <WebPageJsonLd title={`${def.title} | New Grad | Canada | NurseNest`} description={def.tagline} path={hubPath} />
      <BreadcrumbBar crumbs={crumbs} schemaItems={schemaItems} />
      <NewGradWorkAreaHub shell="canada" definition={def} pathway={pathway} overview={overview} study={study} />
    </div>
  );
}
