...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  const { careerSlug: raw } = await params;
  ...
  return safeGenerateMetadata(
    async () => {
      ...
      const canonicalPath = page <= 1 ? `/nursing/${career}/blog` : `/nursing/${career}/blog?page=${page}`;
      return {
        title: `${labels.short} nursing exam prep blog | NurseNest`,
        description: `Clinical and exam-prep articles for ${labels.h1} candidates.`,
    ...
    { pathname: `/nursing/${career}/blog`, routeGroup: "marketing.default.nursing.career.blog" },
  );
}

export default async function NursingCareerBlogIndexPage({ params, searchParams }: Props) {
  const { careerSlug: raw } = await params;
  const career = resolveNursingScopedCareerSlug(raw);
  if (!career) notFound();
  const labels = nursingScopedCareerLabels(career);
          ...
          <ul className="space-y-6">
            {posts.map((p) => (
              <li key={p.slug} className="border-b border-[var(--theme-separator)] pb-6">
                <Link href={`/nursing/${career}/blog/${p.slug}`} className="text-lg font-semibold text-primary hover:underline">
                  {p.title}
                </Link>
                {p.category ? (
          ...
          {totalPages > 1 ? (
            <nav className="mt-10 flex flex-wrap items-center justify-between gap-4 text-sm" aria-label="Blog pagination">
              ...
              <div className="flex gap-3">
                {page > 1 ? (
                  <Link
                    href={page === 2 ? `/nursing/${career}/blog` : `/nursing/${career}/blog?page=${page - 1}`}
                ...
                {page < totalPages ? (
                  <Link href={`/nursing/${career}/blog?page=${page + 1}`} className="font-medium text-primary hover:underline">
                    Next
                  </Link>
                ) : (
