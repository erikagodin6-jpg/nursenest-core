...
import {
  acquireQuestionFullModeSlot,
  releaseQuestionFullModeSlot,
} from "@/lib/questions/full-mode-concurrency";
import {
...
async function pickQuestionIdsByRandomOffset(
  whereSql: Prisma.Sql,
  take: number,
): Promise<string[]> {
  ...
  if (firstRows.length >= limit || offset === 0) {
    return shuffleSeeded(
      ...
      `${randomUUID()}:question-bank-random-offset`,
    ).slice(0, limit);
  }
  ...
  return shuffleSeeded(
    [...firstRows, ...wrapRows].map((r) => r.id),
    `${randomUUID()}:question-bank-random-offset-wrap`,
  ).slice(0, limit);
}
...
function logSubscriberPayload(
  ...
  const payloadLarge =
    approxPayloadBytes >= QUESTION_PAYLOAD_WARN_BYTES ? 1 : 0;
  ...
  if (
    meta.responseMode === "full" ||
    approxPayloadBytes >= QUESTION_LIST_PAYLOAD_LOG_MIN_BYTES
  ) {
...
export async function GET(req: NextRequest) {
  return runWithApiTelemetry(req, "GET /api/questions", "content", async () => {
    ...
    const pageSizeParsed =
      pageSizeRaw === null || pageSizeRaw === "" ? 10 : Number(pageSizeRaw);
    if (!Number.isFinite(pageSizeParsed) || !Number.isInteger(pageSizeParsed)) {
      return NextResponse.json(
        {
          error: "Invalid pageSize (must be an integer).",
    ...
    const practiceHubIdsParsed = parsePracticeHubIdsParam(
      searchParams.get("practiceHubIds"),
    );
    ...
    const alliedProfessionParam =
      searchParams.get("alliedProfession")?.trim().toLowerCase() ?? "";
    ...
    const includeIds = parseCommaSeparatedQuestionIds(
      searchParams.get("includeIds"),
    );
    const focusIds = includeIds.length > 0 ? includeIds : mistakeIds;
    const examFilterRaw = searchParams.get("exam")?.trim() ?? "";
    const examFilter =
      examFilterRaw.length > 0 && examFilterRaw.length <= 64
        ? examFilterRaw
        : null;
    const difficultyMin = parseDifficultyBound(
      searchParams.get("difficultyMin"),
    );
    const difficultyMax = parseDifficultyBound(
      searchParams.get("difficultyMax"),
    );
    const difficultyScopeMode = parseQuestionDifficultyScopeMode(
      searchParams.get("difficultyScope"),
    );
    if (
      difficultyMin != null &&
      difficultyMax != null &&
      ...
      return NextResponse.json(
        {
          error: "difficultyMin must be less than or equal to difficultyMax.",
    ...
    const questionOverlayBundle =
      await resolveMergedQuestionOverlayBundle(educationalLocale);

    if (sort === "random" && page > 1 && excludeIds.length === 0) {
      return NextResponse.json(
        {
          error:
            "Random mode uses page=1, or pass excludeIds for additional batches.",
    ...
    if (skipRows > MAX_QUESTION_LIST_SKIP_ROWS) {
      ...
      return NextResponse.json(
        {
          error:
            "Page too deep for this list. Use topic or pathway filters, or a narrower query.",
    ...
    try {
      ...
      productEvent("entitlement_resolve_failed", {
        surface: "api_questions_list",
      });
      recordEntitlementResolveFailureSignal(
        "api_questions_list",
        correlationId,
      );
      ...
      return NextResponse.json(
        {
          error: "Unable to verify access. Try again shortly.",
          code: "access_verify_failed",
        },
        { status: 503 },
      );
    }
    ...
    if (entitlement.hasAccess) {
      ...
      const rateLimited = await enforceQuestionsListProtection(
        req,
        gate.userId,
        pageSize,
      );
      ...
      if (
        responseMode === "full" &&
        !acquireQuestionFullModeSlot(gate.userId)
      ) {
        safeServerLog("api_questions", "full_mode_concurrency_limit", {
          userId: gate.userId,
        });
        return NextResponse.json(
          {
            error:
              "Too many concurrent full-mode question requests. Try again shortly.",
            code: "rate_limited",
          },
          { status: 429 },
        );
      ...
      try {
        ...
        if (
          pathway &&
          !subscriptionCoversPathwayBase(gate.entitlement, pathway)
        ) {
          pathway = null;
        }
        if (gate.entitlement.tier === "NP") {
          ...
          pathway = resolveNpSubscriberQuestionPathway({
            ...
            pathwayHintIds: [
              userHints?.learnerPath,
              userHints?.targetExamPathwayId,
              gate.userAccess.allowedExam.pathwayId,
            ],
            entitlement: gate.entitlement,
            getExamPathwayById,
          });
        }
        const alliedProfessionExamWhere =
        ...
        if (pathway && practiceHubIdsParsed.length > 0) {
          const ag = await loadPathwayPracticeBodySystemHubAggregates(
            pathway.id,
          );
          ...
          practiceHubOrPrisma = prismaWhereForPracticeHubPool(
            practiceHubTopics,
            practiceHubBodySystems,
          );
        }

        const studyModeRaw =
          searchParams.get("studyMode")?.trim().toLowerCase() ?? "";
        ...
        const effectivePageSize =
          studyMode === "rapid" ? Math.min(pageSize, 8) : pageSize;
        const skipRowsEff =
          sort === "random" ? 0 : (page - 1) * effectivePageSize;

        let topicFilterResolved =
          practiceHubIdsParsed.length > 0 ? "" : (topicFilter ?? "");
        ...
        if (
          studyMode === "weak" &&
          !topicFilterResolved &&
          practiceHubIdsParsed.length === 0
        ) {
          const targets = await getWeakTopicTargetsForPractice(
            gate.userId,
            gate.entitlement,
            4,
          );
          const preferred =
            targets.find((t) => t.confidence !== "low") ?? targets[0];
        ...
        if (gate.entitlement.tier === "NP" && !pathway) {
          const code =
            pathwayIdParam && pathwayIdParam.trim().length > 0
              ? "np_pathway_context_invalid"
              : "np_pathway_id_required";
          return jsonResponseGuarded("/api/questions", {
            ...
            pathwayIdRequested:
              pathwayIdParam && pathwayIdParam.length > 0
                ? pathwayIdParam
                : null,
            topicRequested:
        ...
        const baseWhere = questionAccessWhereWithPathway(
          gate.entitlement,
          pathway,
        );
        ...
        const examExtraSql = examFilter
          ? examEqualsFilterSql(examFilter)
          : Prisma.empty;
        const difficultySqlFrag = difficultyBoundsSql(
          difficultyMin,
          difficultyMax,
        );
        const difficultyScopeSql = questionDifficultyScopeSqlForMode(
          effectiveDifficultyScopeMode,
        );
        const subtopicSql =
        ...
        if (sort === "random") {
          const usePrismaRandomSelection =
            studyModeFilters.length > 0 ||
            pathway?.examFamily === ExamFamily.NP ||
            Boolean(practiceHubOrPrisma);
          if (usePrismaRandomSelection) {
            ...
            questions = shuffleSeeded(
              pool,
              `${randomUUID()}:question-bank-random-page`,
            ...
            const hubSqlActive =
              practiceHubTopics.length > 0 || practiceHubBodySystems.length > 0;
            ...
            let ids = await pickQuestionIdsByRandomOffset(
              Prisma.sql`${accessSql} ${pathSql} ${topicSql} ${subtopicSql} ${examExtraSql} ${difficultySqlFrag} ${difficultyScopeSql} ${focusPoolSql} ${exSql}`,
              effectivePageSize,
            );
            ...
            if (
              ...
              ids = await pickQuestionIdsByRandomOffset(
                Prisma.sql`${accessSql} ${pathSql} ${topicSql} ${subtopicSql} ${examExtraSql} ${difficultySqlFrag} ${difficultyScopeSql} ${focusPoolSql} ${exSql}`,
                effectivePageSize,
              );
              if (ids.length > 0) topicRelaxed = true;
            }
        ...
        let listEmptyDiagnostics:
          | Awaited<ReturnType<typeof diagnoseSubscriberQuestionListEmpty>>
          | undefined;
        if (payload.length === 0) {
          listEmptyDiagnostics = await diagnoseSubscriberQuestionListEmpty({
            entitlement: gate.entitlement,
            pathwayIdParam: pathwayIdParam ?? null,
            topicFilter: topicRelaxed
              ? null
              : topicFilterResolved && topicFilterResolved.length > 0
                ? topicFilterResolved
                : null,
          ...
          safeServerLog("api_questions", "subscriber_list_empty", {
            ...
            entitlementPublished:
              listEmptyDiagnostics.counts.entitlementPublished,
        ...
        const subscriberBody = {
          ...
          pathwayIdRequested:
            pathwayIdParam && pathwayIdParam.length > 0 ? pathwayIdParam : null,
          topicRequested:
            topicFilterResolved && topicFilterResolved.length > 0
              ? topicFilterResolved
              : null,
        ...
        logLargeApiResponse(
          "/api/questions",
          estimateJsonUtf8Bytes(subscriberBody),
        );
        getPaidContentStaleCache().set(
          subscriberQuestionsListStaleKey(
            gate.userId,
        ...
        const staleKey = subscriberQuestionsListStaleKey(
          gate.userId,
          req.nextUrl.searchParams,
        );
        const stale =
          getPaidContentStaleCache().get<Record<string, unknown>>(staleKey);
        if (stale) {
          logDurabilityEvent({
        ...
        return NextResponse.json(
          {
            error: "Unable to load questions. Try again shortly.",
    ...
    if (page > 1) {
      return NextResponse.json(
        {
          error:
            "Complimentary preview is limited to a single page. Subscribe for full question bank access.",
    ...
    const take = Math.min(
      FREEMIUM_QUESTION_LIST_MAX_PER_REQUEST,
      snap.questionRemaining,
    );
    const freemiumFields: QuestionListResponseMode = "preview";

    const freemiumRateLimited = await enforceQuestionsListProtection(
      req,
      userId,
      take,
    );
    ...
    try {
      ...
      const exSql = excludeQuestionIdsSql(excludeIds);
      const ids = await pickQuestionIdsByRandomOffset(
        Prisma.sql`${freeSql} ${freeDifficultyScopeSql} ${exSql}`,
        take,
      );
      const rows = await withRetry(() =>
        prisma.examQuestion.findMany({
      ...
      safeServerLog("api_questions", "freemium_list_payload", {
        ...
        payloadLarge:
          freemiumApproxBytes >= QUESTION_PAYLOAD_WARN_BYTES ? 1 : 0,
      ...
      logLargeApiResponse(
        "/api/questions",
        estimateJsonUtf8Bytes(freemiumBody),
      );
