...
import { medCalcLessonProgressId } from "@/lib/med-calculations/med-calc-lesson-progress";
...
export async function POST(req: Request) {
  return runWithApiTelemetry(req, "POST /api/med-calculations/progress", "content", async () => {
    ...
    setSentryServerContext({
      route: "/api/med-calculations/progress",
    ...
    const medTrack: MedCalcTrack =
      gate.entitlement.tier != null ? medCalcTrackFromTier(gate.entitlement.tier) : (track as MedCalcTrack);

    if (medTrack !== track && gate.entitlement.tier != null) {
      return NextResponse.json({ error: "Track mismatch" }, { status: 403 });
    }

    const lesson = getMedCalcLessonByCategoryAndSlug(category as MedCalcCategorySlug, lessonSlug, medTrack);
    if (!lesson) {
    ...
    const syntheticLessonId = medCalcLessonProgressId(medTrack, lesson);
