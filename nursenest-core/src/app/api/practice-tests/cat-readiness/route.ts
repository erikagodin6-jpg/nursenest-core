...
export async function GET(req: NextRequest) {
  return runWithApiTelemetry(
    req,
    "GET /api/practice-tests/cat-readiness",
    "content",
    async () => {
      const gate = await requireSubscriberSession();
      if (!gate.ok) return gate.response;

      const rl = await enforcePracticeTestsCatReadinessProtection(
        req,
        gate.userId,
      );
      if (rl) return rl;
      ...
      const includeStaffDiagnostics =
        req.nextUrl.searchParams.get("diagnostics") === "staff";
      const staff = includeStaffDiagnostics
        ? await getStaffSession().catch(() => null)
        : null;
      const result = await assessCatPracticeReadinessForPathway(
        gate.userId,
        gate.entitlement,
        pathwayId,
        {
          includeStaffDiagnostics: Boolean(staff),
        },
      );
      logCoreApiStudyDiagnostic({
        ...
        ok: result.ok,
        availableQuestions: result.ok
          ? result.availableQuestions
          : (result.availableQuestions ?? null),
      ...
      if (!result.ok) {
        safeServerLog("cat_readiness", "preflight_not_ready", {
          code: result.code,
          pathwayIdPrefix:
            pathwayId.length > 12 ? `${pathwayId.slice(0, 12)}…` : pathwayId,
