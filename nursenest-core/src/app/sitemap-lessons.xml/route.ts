...
import { SITEMAP_FALLBACK_LESSONS_PATHS } from "@/lib/seo/sitemap-index-children";
import {
...
/**
 * Pathway lesson hub + topic cluster + lesson detail URLs — same collector as legacy merged `/sitemap.xml`,
 * isolated for segmentation. Uses `SITEMAP_PATHWAY_BUDGET_MS` wall clock budget.
 */
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    const lessonStrings = await collectPathwayLessonSeoUrls(origin, { deadlineEpochMs: pathwayLessonDeadlineMs });
    ...
    xml = buildSitemapUrlsetFromAbsoluteUrls(unique.length > 0 ? unique : [{ loc: `${origin}/lessons` }]);
  } catch {
    const fallbackEntries: SitemapUrlEntry[] = SITEMAP_FALLBACK_LESSONS_PATHS.map((path) => ({
