...
const ruleSchema = z.object({
  ...
  trigger: z.enum(["SIGNUP_ATTRIBUTED", "QUALIFIED_REFERRAL_COUNT", "PAID_REFERRAL_COUNT"]),
  threshold: z.coerce.number().int().min(1).max(1000),
  recipient: z.enum(["REFERRER", "FRIEND", "BOTH"]).default("REFERRER"),
  rewardKind: z.enum(["FREE_DAYS", "FREE_MONTH", "ACCOUNT_CREDIT", "FIRST_MONTH_DISCOUNT", "PREMIUM_TRIAL", "FEATURE_UNLOCK", "AMBASSADOR_STATUS", "MANUAL"]),
...
export async function createReferralRewardRule(formData: FormData) {
  ...
  await referralDb.referralRewardRule.create({
    data: {
      ...
      discountPercent: Number.isFinite(parsed.data.discountPercent) ? parsed.data.discountPercent : null,
