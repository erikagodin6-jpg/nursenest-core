...
- **Deploy (App Platform):** the Node buildpack runs `npm run build` first (skips duplicate `next build` on DO when `DIGITALOCEAN_APP_ID` is set); `npm run build:deploy` then verifies the standalone artifact, syncs `.next/static` into the standalone tree, and runs `post-build-prune.mjs` (**preserves** `.next/cache` by default for the next buildpack cache snapshot — opt out with `NN_POST_BUILD_PRUNE_NEXT_CACHE=1`) — **no second next build**. (`build:deploy:app-platform` is an alias of `build:deploy`.) For a **single** command that includes `next build` (droplet/CI), use **`npm run build:deploy:full`**.
