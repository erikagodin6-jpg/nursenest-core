...
- **Deploy (App Platform):** the Node buildpack runs `npm run build` first; `npm run build:deploy` then verifies the standalone artifact, syncs `.next/static` into the standalone tree, and runs `post-build-prune.mjs` (drops `.next/cache`) — **no second next build**. (`build:deploy:app-platform` is an alias of `build:deploy`.) For a **single** command that includes `next build` (droplet/CI), use **`npm run build:deploy:full`**.
