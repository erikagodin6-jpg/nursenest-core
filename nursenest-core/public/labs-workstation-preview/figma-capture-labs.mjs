#!/usr/bin/env node
/**
 * Capture Labs workstation into Figma (requires local dev server + auth cookie if gated).
 * Usage:
 *   node public/labs-workstation-preview/figma-capture-labs.mjs <captureId> [hub|lesson] [theme] [--mobile]
 *
 * Examples:
 *   node public/labs-workstation-preview/figma-capture-labs.mjs nn-labs-hub-ocean-2026 hub ocean
 *   node public/labs-workstation-preview/figma-capture-labs.mjs nn-labs-lesson-mint-2026 lesson mint-blossom --mobile
 */
...
if (!captureId) {
  console.error(
    "usage: node figma-capture-labs.mjs <captureId> [hub|lesson] [theme] [--mobile]",
  );
  process.exit(1);
}
...
const paths = {
  hub: "/app/labs",
  lesson: "/app/labs/electrolytes/potassium-priority-management",
};
const path = paths[surface] ?? paths.hub;
const url = `http://127.0.0.1:3000${path}`;
const viewport = mobile ? { width: 390, height: 844 } : { width: 1440, height: 900 };
const selector = "[data-nn-labs-workstation]";
