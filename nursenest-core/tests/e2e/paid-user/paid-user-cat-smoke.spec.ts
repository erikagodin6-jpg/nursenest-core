...
const RN_CAT_PATHWAY_IDS = [...new Set([PAID_E2E_DEFAULT_PATHWAY_ID, "ca-rn-nclex-rn"])];
...
test.describe("Paid user — CAT smoke", () => {
  for (const pathwayId of RN_CAT_PATHWAY_IDS) {
    test(`practice hub → begin exam → 3 items (${pathwayId})`, async ({ page }) => {
      ...
      try {
        ...
        logPaidSurfaceDebug(buildPaidFailureSnapshot(await collectPaidSurfaceDebug(page, `cat-smoke:${pathwayId}`), obs));
        throw e;
      } finally {
        obs.dispose();
      }
