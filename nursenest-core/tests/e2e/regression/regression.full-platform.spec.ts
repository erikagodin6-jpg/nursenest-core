 ...
 *
 * Run via `npm run test:e2e:full-regression` (see `playwright.full-regression.config.ts`).
 *
...
test.describe("@slow full-platform regression (guest)", () => {
  ...
  test("guest-cat-ecg-labs-probes — marketing CAT surface, ECG marker, med-math tool @slow", async ({
    ...
    const hasInput =
      (await page.locator('input[type="text"], input[inputmode="decimal"]').first().isVisible().catch(() => false)) ||
      (await page.locator("input").count()) > 0;
    expect(hasInput, "med-math has an input").toBeTruthy();
