 ...
 * session affordances (dashboard, account menu, sign out) remain after hydration.
 *
    ...
    test(`${code}: marketing public shell; dashboard + account menu`, async ({ page }, testInfo) => {
      ...
      try {
        ...
        const accountBtn = page
          ...
          .first();
        await expect(accountBtn).toBeVisible({ timeout: 60_000 });
        ...
        await expect(page.getByRole("menu").locator('a[href="/app"]').first()).toBeVisible({ timeout: 15_000 });
        await expect(page.locator('[role="menu"] button[role="menuitem"]').last()).toBeVisible({
          timeout: 10_000,
        });
