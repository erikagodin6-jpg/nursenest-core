...
async function expectIndexLinkWithinPages(page: Page, slug: string): Promise<void> {
  ...
  for (let p = 1; p <= FIRST_BATCH_INDEX_LINK_MAX_PAGES; p += 1) {
    ...
    const res = await gotoOkWithRetries(page, listPath);
...
/** Transient 5xx / cold-compile flakes on local `next dev` + remote DB. */
async function gotoOkWithRetries(page: Page, path: string, attempts = 3): Promise<Awaited<ReturnType<Page["goto"]>>> {
  let last: Awaited<ReturnType<Page["goto"]>> = null;
  for (let i = 0; i < attempts; i += 1) {
    last = await page.goto(path, { waitUntil: "domcontentloaded", timeout: 120_000 });
    if (last?.ok()) return last;
    if (i + 1 < attempts) await sleep(3000);
  }
  return last;
}
...
test.describe("Blog patho/pharm smoke (static slugs)", () => {
  ...
  test("blog index loads", async ({ page }) => {
    const res = await gotoOkWithRetries(page, "/blog");
  ...
  test("NCLEX-PN marketing hub (US PN) loads for PN cohort smoke", async ({ page }) => {
    ...
    if (!res?.ok()) {
      test.skip(true, `${url} returned ${st} (blog smoke allows transient marketing hub failure)`);
    }
    await expect(page.locator("main")).toBeVisible();
  });
