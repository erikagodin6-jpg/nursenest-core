 ...
 * placeholder-shaped copy guardrails, public marketing theme picker scope (Ocean + Midnight),
...
test.describe("Premium smoke breadth (public)", () => {
  ...
  test("public marketing theme picker lists only allowlisted themes (desktop)", async ({ page }) => {
    ...
    await expect(options.filter({ hasText: /^Ocean$/ })).toHaveCount(1);
    await expect(options.filter({ hasText: /^Midnight$/ })).toHaveCount(1);
  });
