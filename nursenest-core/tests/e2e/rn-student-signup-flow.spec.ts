...
test.describe("RN student signup flow", () => {
  test("homepage → US RN hub → signup → onboarding → learner shell + RN nav + paywall", async ({ page, baseURL }) => {
    ...
    await expect(page).toHaveURL(/\/us\/rn\/nclex-rn/i, { timeout: 25_000 });
    await expectNotPageNotFound(page);
    ...
    const email = `rn-student-e2e+${stamp}@${domain}`;
