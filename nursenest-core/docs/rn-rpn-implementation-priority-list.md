...
**G11 note:** English marketing hubs (US RN, CA RN, US PN, CA REx-PN) now have **sentinel regression** coverage in `tests/e2e/public/pn-marketing-hub-i18n-sanity.spec.ts`. Incomplete **fr/es** locale copy remains a separate **AI_CAN_PREP_BUT_DEV_SHOULD_REVIEW** item tracked in `i18n-route-readiness.spec.ts`.
