...
External dashboards: **Stripe** webhook → **`https://www.nursenest.ca/api/subscriptions/webhook`** for NurseNest production (TLS + signing secret: `docs/stripe-webhook-production-operations.md`); other envs: `https://<your-domain>/api/subscriptions/webhook` with valid TLS for that host. **Inngest** app sync URL → `https://<your-domain>/api/inngest`; **Resend** verified domain; **Spaces** CORS/keys; **Sentry** DSN + optional release auth token.
