...
| Marketing lesson / CAT / flashcards down | Synthetic `marketing_lesson_detail`, `marketing_cat_launch`, or `marketing_flashcards` non-2xx | Per cron + 5m | `synthetic.check.failed` with those `check` names | platform-engineering | 1) Open the pathway URL from `synthetic-monitored-paths.ts`. 2) Check content/DB for lesson slug drift. 3) Compare with `deploy-learner-smoke` Playwright spec. | [Content](#content-runbook) |
...
| Checkout / pricing path down | Synthetic `pricing_options` / `pricing_page` non-2xx, `checkout_api_unauthenticated` returns 5xx, OR billing 5xx spike | Per cron + 5m | `synthetic.check.failed` `{ check: pricing_options \| pricing_page \| checkout_api_unauthenticated }` + `api.route.count` `5xx` `flow:billing` | platform-engineering | 1) `GET /api/pricing/options`. 2) Stripe keys + `NEXT_PUBLIC_APP_URL`. 3) Test `POST /api/subscriptions/checkout` in staging. 4) Check `billing.checkout.failure` reasons. | [Billing](#billing-runbook) |
| Stripe webhook handler down | Synthetic `stripe_webhook_invalid_body` returns 5xx (handler crash) | Per cron + 5m | `synthetic.check.failed` `{ check: stripe_webhook_invalid_body }` | platform-engineering | 1) Stripe dashboard delivery log. 2) `STRIPE_WEBHOOK_SECRET` + route logs. 3) Pair with `billing.webhook.failure` metric. | [Webhooks](#webhooks-runbook) |
| Lesson API gate broken | Synthetic `api_lessons_gate` returns 5xx (expected **401** when unauthenticated) | Per cron + 5m | `synthetic.check.failed` `{ check: api_lessons_gate }` | platform-engineering | 1) `GET /api/lessons?limit=1` signed out → expect 401. 2) If 5xx, follow [Entitlements](#entitlements-runbook) + Prisma logs. | [Content](#content-runbook) |
...
- Surfaces: `page` (RSC `resolveEntitlementForPage`), `api_questions_id` (single-question API), `api_questions_list` / `api_lessons_list` (bulk list routes when `resolveEntitlement` throws), `subscriber_api` (`requireSubscriberSession` when `getUserAccess` throws → 503).
...
| Synthetic latency regression | `synthetic.probe.duration_ms` p95 high for `check:marketing_home` (or other checks) | 1h | `synthetic.probe.duration_ms` `{ check, surface, traffic_source:synthetic }` | platform-engineering | 1) Compare to `ALERT_THRESHOLDS.synthetic.slow*ProbeMs`. 2) Check `synthetic.check.slow` spikes. 3) DB cold start vs CDN — tune thresholds in `alert-thresholds.ts` first. | [Homepage](#homepage-runbook) |
