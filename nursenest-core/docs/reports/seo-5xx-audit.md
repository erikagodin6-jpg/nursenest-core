...
| Sitemap children | 25 child sitemap urlsets are emitted by the index. | Medium if child route fetches DB without fallback | Verify each child with live sitemap checks. |
...
- /admin/growth-revenue-command-center (src/app/(admin)/admin/growth-revenue-command-center/page.tsx)
...
- /admin/navigation-compliance (src/app/(admin)/admin/navigation-compliance/page.tsx)
