...
function render(spec: PostSpec): string {
  ...
  const safetyChecklist = `<h2>Clinical safety checklist before you move on</h2>
<p>Before leaving the bedside or moving to a teaching answer, ask whether you have enough data to know the patient is not actively deteriorating. Recheck the most relevant vital sign, pain or neuro finding, oxygenation cue, intake-output change, rhythm concern, or bleeding risk. Nursing judgment is strongest when it links a named condition to an observable patient response.</p>
<p>Also ask what could make the usual intervention unsafe. Fluids can worsen pulmonary edema, oxygen plans need ordered targets in chronic lung disease, anticoagulants require bleeding surveillance, sodium correction can injure the brain, and sedating medications can hide neurologic decline. This pause is not hesitation; it is the clinical reasoning step that separates safe prioritization from memorized routines. When the stem changes, repeat the safety scan instead of carrying forward yesterday's priority.</p>`;
  const draftLine = PUBLISH ? "" : "draft: true\n";
...
console.log(`Wrote ${posts.length} ${PUBLISH ? "published" : "draft"} clinical long-tail posts to ${OUT_DIR}`);
