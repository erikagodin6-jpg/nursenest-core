...
def build_body(slug: str, title: str, knack: str, cluster: str) -> str:
    topic_sentence = (
        f"This article focuses on {knack.lower()} for paramedics and AEMTs, emphasizing how field clinicians "
    ...
    for base, cluster, tails in TOPIC_MATRIX:
        for tail in tails:
            ...
            ctx = (
                ...
                .replace("Mci ", "MCI ")
                .replace("Smr ", "SMR ")
                .replace("Ecg Ems", "ECG EMS")
                .replace("Svt Vt Wide Complex", "SVT/VT (wide-complex)")
            )
            knack = f"{slugify_title(tail)} ({ctx})"
            ...
            report_lines.append(f"| `{rel}` | `{slug}` | {wc} | pass (gates in table above) |")
    report_lines.append("")
