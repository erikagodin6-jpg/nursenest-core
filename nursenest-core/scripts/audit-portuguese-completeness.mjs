...
const CONTROLLED_ENGLISH_TERMS = /\b(?:NurseNest|REx-PN|NCLEX(?:-[A-Z]+)?|CPNRE|OSCE|CAT|ECG|EKG|IV|BP|RN|RPN|PN|NP|NGN|NCSBN|CASN|LPN|LVN|NNQE|UWorld|Archer|Klarna|Afterpay|Affirm|LinkedIn|Allied Health|Medical Imaging|New Grad|Stripe|Flashcards?|flashcards?|SpO2|PaCO2|PaO2|HCO|mmHg|mEq|mol|dL)\b/gi;

const SURFACES = [
  { route: "/pt", surfaceType: "homepage", prefixes: ["pages.home.", "nav.", "footer.", "brand."], indexable: true },
  { route: "/pt/pricing", surfaceType: "pricing", prefixes: ["pages.pricing.", "pricing.", "nav.", "footer."], indexable: true },
  { route: "/pt/rn", surfaceType: "rn-hub", prefixes: ["pages.exam", "pages.pathway", "components.examPathwayHub.", "nav.", "footer."], indexable: true },
  { route: "/pt/rex-pn", surfaceType: "rex-pn-hub", prefixes: ["pages.exam", "pages.pathway", "components.examPathwayHub.", "nav.", "footer."], indexable: true },
  { route: "/pt/np", surfaceType: "np-hub", prefixes: ["pages.exam", "pages.pathway", "components.examPathwayHub.", "nav.", "footer."], indexable: true },
  { route: "/pt/allied", surfaceType: "allied-hub", prefixes: ["allied.alliedHealthHub.", "pages.allied", "nav.", "footer."], indexable: true },
  { route: "/pt/new-grad", surfaceType: "new-grad-hub", prefixes: ["pages.newGrad", "newGrad.", "nav.", "footer."], indexable: true },
  { route: "/pt/lessons", surfaceType: "lesson-library", prefixes: ["pages.lessons.", "lesson.", "lessons.", "nav.", "footer."], indexable: true },
  { route: "/pt/question-bank", surfaceType: "practice-questions", prefixes: ["pages.publicQuestionBank.", "pages.questionBank.", "questions.", "practice.", "nav.", "footer."], indexable: true },
  ...
  { route: "/pt/login", surfaceType: "auth", prefixes: ["auth.", "account.", "nav.", "common."], indexable: false },
];
...
function englishLeak(value) {
  ...
  const hasPortugueseEvidence = /[áàâãéêíóôõúç]|(?:\b(?:de|do|da|dos|das|se|que|para|com|seu|sua|exame|enfermagem|perguntas|questões|preparação|lições|cuidados|paciente|clínico|clínica|força|risco|pagamento|acesso|estudo|prática)\b)/i.test(text);
...
const pt = loadLocale(LOCALE);
const missingShards = SHARDS.filter((shard) => !fs.existsSync(path.join(I18N_ROOT, LOCALE, `${shard}.json`)));
const missingKeys = Object.keys(en).filter((key) => !(key in pt) || String(pt[key] ?? "").trim() === "");
const extraKeys = Object.keys(pt).filter((key) => !(key in en));
const placeholderFields = Object.entries(pt).filter(([, value]) => typeof value === "string" && hasPlaceholder(value)).map(([key]) => key);
const untranslatedFields = Object.keys(en).filter((key) => pt[key] === en[key] && !isProtectedIdentical(pt[key]));
const englishLeakSuspicions = Object.entries(pt)
...
const surfaceReports = SURFACES.map((surface) => {
  ...
  return {
    ...
    recommendedFix: missing.length || placeholders.length || untranslated.length || leaks.length
      ? "Fix missing Portuguese keys/placeholders/English fallback fields before indexing this pt-BR surface."
      : "Portuguese surface is structurally complete.",
  };
});
...
const report = {
  ...
  totalPortugueseKeys: Object.keys(pt).length,
  missingShards,
  missingKeys,
  extraKeys,
  ...
  portugueseSnapshot: snapshotLocale(LOCALE),
...
const reportName = IS_BEFORE ? "i18n-pt-audit-before.json" : "i18n-pt-audit-after.json";
fs.mkdirSync(REPORT_DIR, { recursive: true });
fs.writeFileSync(path.join(REPORT_DIR, reportName), `${JSON.stringify(report, null, 2)}\n`, "utf8");

if (!IS_BEFORE) {
  const md = [
    "# Portuguese (Brazil) I18n Summary",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    `Coverage: ${coveragePct}% (${report.totalPortugueseKeys}/${report.totalEnglishKeys})`,
    ...
    "## SEO Validation",
    "",
    "- Portuguese breadcrumbs, localized slugs, metadata readiness, hreflang, canonical, sitemap, and JSON-LD are checked by `npm run audit:localized-seo`.",
    "- Playwright route verification is checked by `npm run test:localized-seo:routes` when the Next.js dev server can compile.",
    "",
  ].join("\n");
  fs.writeFileSync(path.join(REPORT_DIR, "i18n-pt-summary.md"), md, "utf8");
}
...
console.log(`[i18n:pt] wrote reports/${reportName}; coverage=${coveragePct}% untranslated=${untranslatedFields.length}`);
