...
const REQUIRED_RENAL_TOPICS: Array<{ label: string; keys: string[] }> = [
  { label: "AKI vs CKD", keys: ["aki", "ckd", "acute kidney injury", "chronic kidney disease"] },
  { label: "dialysis", keys: ["dialysis", "hemodialysis", "peritoneal dialysis"] },
  { label: "electrolyte imbalances", keys: ["electrolyte", "hyperkalemia", "hypokalemia", "hyponatremia", "hypernatremia"] },
  { label: "fluid balance", keys: ["fluid balance", "i&o", "oliguria", "anuria"] },
];

const RENAL_FOCUS_KEYS = [
  "renal",
  "kidney",
  "neph",
  "aki",
...
const RENAL_EXCLUDE_KEYS = ["fetal kidney", "obstetric", "labor", "postpartum"];

function isRenalText(text: string): boolean {
  const t = text.toLowerCase();
  if (RENAL_EXCLUDE_KEYS.some((k) => t.includes(k))) return false;
  return RENAL_FOCUS_KEYS.some((k) => t.includes(k));
}
...
async function main() {
  ...
  const renalRows = allRows.filter((r) => isRenalText(`${r.title} ${r.topic} ${r.bodySystem} ${r.slug}`));
  const corpus = renalRows.map((r) => `${r.title} ${r.topic} ${r.bodySystem} ${r.slug}`.toLowerCase()).join("\n");
  const missingTopics = REQUIRED_RENAL_TOPICS.filter((t) => !t.keys.some((k) => corpus.includes(k.toLowerCase()))).map((t) => t.label);
    ...
    JSON.stringify(
      {
        ...
        missingRenalTopics: missingTopics,
      },
      null,
      2,
    ),
