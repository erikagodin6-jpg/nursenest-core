#!/usr/bin/env npx tsx
/**
 * Publishes new-grad nursing production batch 03 from
 * ../../data/blog-content/newgrad-prod-batch-03/ (repo root) into BlogPost (Prisma).
 *
 * Usage (from this package directory):
 *   npx tsx scripts/publish-newgrad-manifest-batch-03.ts
 *   npx tsx scripts/publish-newgrad-manifest-batch-03.ts --dry-run
 *
 * Does not overwrite published posts (skips if slug exists).
 * Processes one post at a time (sequential loop).
 *
 * 50 articles in this batch — newgrad nursing scenarios across specialties.
 */
...
const BATCH_DIR = path.join(REPO_ROOT, "data/blog-content/newgrad-prod-batch-03");
...
const APA = [
  ...
  "Rhodes, A., et al. (2017). Surviving Sepsis Campaign: International Guidelines for Management of Sepsis and Septic Shock: 2016. Intensive Care Medicine, 43(3), 304-377. https://doi.org/10.1007/s00134-017-4681-6",
...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(INDEX_PATH)) {
    console.error(JSON.stringify({ ok: false, error: "index.json not found", path: INDEX_PATH }));
    process.exit(1);
  }
  ...
  console.log(JSON.stringify({ starting: true, batch: "newgrad-prod-batch-03", total: rows.length, dryRun }));

  for (let i = 0; i < rows.length; i += 1) {
    const row = rows[i];
    const bodyNum = String(i + 1).padStart(2, "0");
    const bodyPath = path.join(BATCH_DIR, `body-${bodyNum}.html`);

    if (!fs.existsSync(bodyPath)) {
      console.error(JSON.stringify({ ok: false, error: "missing_body_file", path: bodyPath, index: i }));
      process.exit(1);
    }

    let bodyHtml = fs.readFileSync(bodyPath, "utf8");

    // Wire related reading block
    ...
    if (wc < 800) {
      skipped.push({ slug: row.slug, reason: `body_below_800_words_${wc}` });
      console.warn(JSON.stringify({ warn: "low_word_count", slug: row.slug, words: wc }));
      continue;
    }
    ...
    const faqBlock = {
      items: [
        {
          q: `What is the priority action for new grads when this situation occurs?`,
          a: "Stabilize the immediate patient safety threat within your scope, collect objective data, and communicate early when the clinical trajectory is worsening.",
        },
        {
          q: "When should I escalate even if uncertain?",
          a: "Escalate when you observe high-risk patterns, persistent abnormal trends, or when your clinical assessment suggests the patient is deteriorating faster than you can safely manage alone.",
        },
        {
          q: "How does this apply to NCLEX-style questions?",
          a: "Boards test priority assessment, safe escalation, scope-appropriate delegation, and closed-loop communication — the same skills needed at the bedside.",
        },
      ],
    };
    ...
    const data: Prisma.BlogPostCreateInput = {
      ...
      apaReferences: APA,
      legacySource: "newgrad-prod-batch-03",
      shortSummary: row.seoDescription.slice(0, 220),
    };

    if (dryRun) {
      console.log(JSON.stringify({ dryRun: true, wouldCreate: row.slug, words: wc, index: i + 1 }));
    } else {
      await prisma.blogPost.create({ data });
      console.log(JSON.stringify({ ok: true, created: row.slug, words: wc, index: i + 1 }));
    }
