...
function parseArgs() {
  ...
  const only = onlyRaw
    ? (onlyRaw.split(",").map((s) => s.trim().toLowerCase()) as StreamKind[])
    : null;
  return {
    source: source ? path.resolve(source) : "",
    apply: argv.includes("--apply"),
    batchSize: Math.min(BATCH_MAX, Math.max(1, Number(get("batch-size") ?? BATCH_MAX))),
    checkpointPath: get("checkpoint") ? path.resolve(get("checkpoint")!) : DEFAULT_CHECKPOINT,
    only: only && only.length ? new Set(only as StreamKind[]) : null,
...
async function main() {
  const { source, apply, batchSize, checkpointPath, only, quarantineDir } = parseArgs();
  ...
  const appendQ = (line: string) => fs.appendFileSync(quarantineLog, line + "\n");
      ...
      for (const file of files) {
        ...
        for await (const { idx, row } of iterateFileRecords(file)) {
          ...
          try {
            ...
            if (!pathwayId) {
              appendQ(JSON.stringify({ key, reason: "no_pathway", row }));
              counts.errors += 1;
              processed.add(key);
              continue;
            }
            ...
            if (!Array.isArray(sections) || sections.length === 0) {
              appendQ(JSON.stringify({ key, reason: "missing_sections", slug }));
              counts.errors += 1;
              processed.add(key);
              continue;
            }
            ...
            counts.errors += 1;
            appendQ(JSON.stringify({ key, err: e instanceof Error ? e.message : String(e) }));
            processed.add(key);
          }
        }
        if (buf.length) {
          ...
          try {
            ...
            if (kind === "pool" || kind === "practice" || kind === "practice_pool") {
              ...
              if (!pathwayId || !lessonSlug || !items?.length) {
                appendQ(JSON.stringify({ key, reason: "lesson_quiz_incomplete", pathwayId, lessonSlug }));
                counts.errors += 1;
                processed.add(key);
                continue;
              }
              if (apply) {
                ...
                if (!rowL) {
                  appendQ(JSON.stringify({ key, reason: "lesson_not_found", pathwayId, lessonSlug }));
                  counts.errors += 1;
                } else {
