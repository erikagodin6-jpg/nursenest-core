...
const SOURCE = "cnple-launch-readiness-sprint";
const CATEGORY_SLUG = "cnple-launch-readiness";
const DECK_SLUG = "cnple-clinical-reasoning-launch-deck";
...
function teachingRationale(lesson: CatalogLesson, domain: Domain, correct: string): string {
  return [
    ...
    `Why alternatives are tempting: ${domain.tempting}. The CNPLE trap is ${domain.trap}.`,
...
function optionRationales(optionMap: Record<string, string>, correctLetter: string, domain: Domain): Record<string, string> {
  return Object.fromEntries(
    Object.entries(optionMap).map(([letter, text]) => {
      ...
      return [
        letter,
        `${letter} is incorrect. ${text} is tempting because ${domain.tempting}, but it misses ${domain.cue}, tests the CNPLE trap of ${domain.trap}, and fails to apply this NP reasoning: ${domain.reasoning}.`,
      ];
    }),
  );
}
...
function buildMcq(lesson: CatalogLesson, domain: Domain, localIndex: number, globalIndex: number): QuestionRow {
  ...
  const stem = `CNPLE ${domain.label} item ${localIndex + 1}. A nurse practitioner sees ${domain.presentation} while applying ${lesson.title}. Key cue: ${domain.cue}. Which plan best demonstrates advanced NP clinical reasoning?`;
  return questionRow({
    id: `cnple-${slug(lesson.slug)}-q-${String(localIndex + 1).padStart(3, "0")}`,
    lesson,
    domain,
    stem,
    questionType: "MCQ",
...
function buildSata(lesson: CatalogLesson, domain: Domain, localIndex: number, globalIndex: number): QuestionRow {
  ...
  return questionRow({
    ...
    rationale: `A-D are correct because CNPLE-level care requires differential diagnosis, targeted diagnostic interpretation, management reasoning, and prescribing safety. E is premature closure. F is low-value testing unless each test changes probability, safety, treatment, or follow-up.`,
    difficulty: difficulty(globalIndex),
...
function buildOrdered(lesson: CatalogLesson, domain: Domain, localIndex: number, globalIndex: number): QuestionRow {
  ...
  return questionRow({
    ...
    rationale: `CNPLE reasoning moves from differential diagnosis to diagnostic confirmation, management decision, and prescribing/monitoring. The trap is prescribing or reassuring before diagnostic risk is resolved.`,
    difficulty: difficulty(globalIndex),
...
function buildClinicalManagement(lesson: CatalogLesson, domain: Domain, localIndex: number, globalIndex: number): QuestionRow {
  ...
  return questionRow({
    ...
    lesson,
    domain,
    stem: `Clinical management. Which management plan is safest for ${domain.presentation} in a CNPLE-style ${lesson.title} encounter?`,
    questionType: "MCQ",
...
function buildDiagnosticInterpretation(lesson: CatalogLesson, domain: Domain, localIndex: number, globalIndex: number): QuestionRow {
  ...
  return questionRow({
    ...
    lesson,
    domain,
    stem: `Diagnostic interpretation. A result is available during ${lesson.title}. Which interpretation best reflects CNPLE-level NP reasoning?`,
    questionType: "MCQ",
...
function questionRow(input: {
  ...
  return {
    ...
    clinicalPearl: `CNPLE pearl: ${input.domain.cue} should change the differential, diagnostic plan, management plan, prescribing safety check, or follow-up interval.`,
    examStrategy: "For CNPLE, avoid RN-level recall. Identify the differential, interpret targeted data, prescribe safely, and decide whether outpatient management is safe.",
    ...
    referenceSource: "NurseNest CNPLE lesson-derived launch readiness sprint",
...
function flashcardRow(lesson: CatalogLesson, domain: Domain, index: number, categoryId: string, deckId: string, sourceSuffix: string): FlashcardRow {
  ...
  const front =
    mode === "case"
      ? `CNPLE case: In ${lesson.title}, how should the NP reason through ${domain.presentation} when the cue is ${domain.cue}?`
      : `CNPLE ${mode}: What is the key NP-level decision in ${lesson.title} when ${domain.cue} is present?`;
  const back = [
  ...
  return {
    id: `cnple-fc-${String(index + 1).padStart(5, "0")}`,
    front,
    back,
    ...
    sourceKey: `cnple:flashcard:${sourceSuffix}`,
    examItemKind: null,
  };
}
...
async function createPracticeExams(
  ...
  for (let i = 1; i <= EXAM_TARGET; i++) {
    ...
    const examId = `exam_cnple_launch_${padded}`;
    const tag = `exam-preset-cnple-launch-${padded}`;
    await prisma.exam.upsert({
      where: { id: examId },
      update: { title: `CNPLE Blueprint Practice Exam ${i}`, status: ContentStatus.PUBLISHED },
      create: {
        id: examId,
        title: `CNPLE Blueprint Practice Exam ${i}`,
        country: CountryCode.CA,
        tier: TierCode.NP,
        examFamily: ExamFamily.NP,
        status: ContentStatus.PUBLISHED,
...
async function main(): Promise<void> {
  ...
  try {
    const category = await prisma.category.upsert({
      ...
      update: { name: "CNPLE Launch Readiness", topicCode: "cnple-launch-readiness" },
      create: { slug: CATEGORY_SLUG, name: "CNPLE Launch Readiness", topicCode: "cnple-launch-readiness" },
    });
    const deck = await prisma.flashcardDeck.upsert({
      where: { slug: DECK_SLUG },
      update: {
        title: "CNPLE Advanced NP Clinical Reasoning Deck",
        description: "Lesson-derived Canadian NP flashcards focused on differential diagnosis, diagnostics, prescribing, preventive care, professional practice, and clinical management.",
        country: CountryCode.CA,
        tier: TierCode.NP,
        examFamily: ExamFamily.NP,
    ...
    console.log(`Publishing ${questions.length} CNPLE questions...`);
    ...
    console.log(`Publishing ${flashcards.length} CNPLE flashcards...`);
    ...
    console.log(`CNPLE launch readiness content published: questions=${questions.length}, flashcards=${flashcards.length}, adaptivePool=${pool.length}, practiceExams=${EXAM_TARGET}`);
  } finally {
