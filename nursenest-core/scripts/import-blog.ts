 ...
 *
 * Run from this package directory (`nursenest-core/`) so `@prisma/client` resolves.
 *
 * Usage:
 * - `npx tsx scripts/import-blog.ts` (apply mode)
 * - `npx tsx scripts/import-blog.ts --dry-run` (no writes)
 * - `npx tsx scripts/import-blog.ts --batch-size=25` (default 25 rows per batch)
 * - `npx tsx scripts/import-blog.ts --dedupe-report` → writes data/audit/blog-deduplication-report.json
 * - `npx tsx scripts/import-blog.ts --dedupe-report=/path/to/out.json`
 */
...
type RunStats = {
  ...
  /** Existing DB row kept because it is longer/richer than incoming legacy row (no overwrite). */
  skippedInferiorIncoming: number;
  dryRunCreates: number;
  dryRunUpdates: number;
  dryRunSkippedInferior: number;
  failures: number;
  failureReasons: string[];
};

type DedupeReportEntry = {
  source: SourceName;
  sourceId: string;
  canonicalSlug: string;
  matchedBy: "sourceKey" | "slug" | "hash" | null;
  action:
    | "created"
    | "updated"
    | "unchanged"
...
function wordCountFromContent(s: string): number {
  return stripHtmlLite(s)
    .split(/\s+/)
    .filter(Boolean).length;
}

function titleWordJaccardSimilarity(a: string, b: string): number {
...
/**
 * When a legacy row matches an existing post, prefer keeping the DB row if it is clearly richer
 * (avoids overwriting newer or higher-quality content with fragments).
 */
function preserveExistingOverIncoming(existing: ExistingPost, row: NormalizedBlogRow): boolean {
  const ew = wordCountFromContent(existing.body);
  const iw = wordCountFromContent(row.content);
  if (iw < 120 && ew > 400) return true;
  if (ew >= 350 && iw < ew * 0.82) return true;
  if (ew > iw + 200) return true;
  const sim = titleWordJaccardSimilarity(existing.title, row.title);
  if (sim >= 0.9 && ew > iw + 80) return true;
  return false;
}
    ...
    JSON.stringify(
      {
        ...
        skippedInferiorIncoming: batchStats.skippedInferiorIncoming,
        dryRunCreates: batchStats.dryRunCreates,
        dryRunUpdates: batchStats.dryRunUpdates,
        dryRunSkippedInferior: batchStats.dryRunSkippedInferior,
...
function pushDedupe(
  dedupeOut: DedupeReportEntry[] | null,
  row: NormalizedBlogRow,
  canonicalSlug: string,
  rest: Omit<DedupeReportEntry, "source" | "sourceId" | "canonicalSlug">,
): void {
  if (!dedupeOut) return;
  dedupeOut.push({ source: row.source, sourceId: row.sourceId, canonicalSlug, ...rest });
}
...
async function processBatch(
  ...
  for (const row of batch) {
    ...
    let matchedBy: "sourceKey" | "slug" | "hash" | null = null;
    if (state.bySourceKey.has(sourceKey)) {
      existing = state.bySourceKey.get(sourceKey)!;
      matchedBy = "sourceKey";
    } else if (state.bySlug.has(canonicalSlug)) {
      existing = state.bySlug.get(canonicalSlug)!;
      matchedBy = "slug";
    } else if (state.byHash.has(recordHash)) {
      existing = state.byHash.get(recordHash)!;
      matchedBy = "hash";
    }

    const nextTags = [...new Set([...(row.seoKeywords ?? []), sourceTag])];
    const ew = existing ? wordCountFromContent(existing.body) : 0;
    const iw = wordCountFromContent(row.content);
    const titleSim = existing ? titleWordJaccardSimilarity(existing.title, row.title) : undefined;
    ...
    if (existing) {
      ...
      if (!payloadChanged) {
        ...
        pushDedupe(dedupeOut, row, canonicalSlug, {
          matchedBy,
          action: dryRun ? "would_skip_unchanged" : "unchanged",
          existingId: existing.id,
          existingWordCount: ew,
          incomingWordCount: iw,
          titleSimilarity: titleSim,
        });
        continue;
      }

      const skipInferior = preserveExistingOverIncoming(existing, row);
      if (skipInferior) {
        if (dryRun) run.dryRunSkippedInferior += 1;
        else run.skippedInferiorIncoming += 1;
        run.totalProcessed += 1;
        pushDedupe(dedupeOut, row, canonicalSlug, {
          matchedBy,
          ...
          incomingWordCount: iw,
          titleSimilarity: titleSim,
          detail: "Existing post kept (longer/richer or clearly more complete).",
        });
        continue;
      }
      ...
      try {
        ...
        run.failureReasons.push(`${row.source}:${row.sourceId} -> ${msg}`);
        pushDedupe(dedupeOut, row, canonicalSlug, {
          matchedBy,
          action: "failed",
          existingId: existing.id,
...
function parseArgs(): {
  dryRun: boolean;
  batchSize: number;
  dedupeReportPath: string | null;
} {
  const dryRun = process.argv.includes("--dry-run");
  let batchSize = BATCH_MAX_DEFAULT;
  const bsArg = process.argv.find((a) => a.startsWith("--batch-size="));
  if (bsArg) {
    const n = Number(bsArg.split("=", 2)[1]);
    if (Number.isFinite(n) && n >= 1 && n <= 100) batchSize = Math.floor(n);
  }
  let dedupeReportPath: string | null = null;
  const drArg = process.argv.find((a) => a.startsWith("--dedupe-report="));
  if (drArg) {
    dedupeReportPath = path.resolve(drArg.split("=", 2)[1]!.trim());
  } else if (process.argv.includes("--dedupe-report")) {
    dedupeReportPath = path.join(__dirname, "../../data/audit/blog-deduplication-report.json");
  }
  return { dryRun, batchSize, dedupeReportPath };
}

async function main() {
  const { dryRun, batchSize: initialBatchSize, dedupeReportPath } = parseArgs();
  const state = await loadExistingDedupState();
  const dedupeOut: DedupeReportEntry[] | null = dedupeReportPath ? [] : null;
  ...
  const run: RunStats = {
    ...
    updated: 0,
    unchangedSkipped: 0,
    skippedInferiorIncoming: 0,
    dryRunCreates: 0,
    dryRunUpdates: 0,
    dryRunSkippedInferior: 0,
    failures: 0,
    failureReasons: [],
  };
  ...
  for await (const row of allSourceRows()) {
    ...
    run.batchNumber += 1;
    await processBatch(batch, state, run, dryRun, dedupeOut);
  ...
  if (dedupeReportPath && dedupeOut) {
    const summary = {
      generatedAt: new Date().toISOString(),
      dryRun,
      batchSizeInitial: initialBatchSize,
      totalRows: dedupeOut.length,
      byAction: {} as Record<string, number>,
    };
    for (const e of dedupeOut) {
      summary.byAction[e.action] = (summary.byAction[e.action] ?? 0) + 1;
    }
    fs.mkdirSync(path.dirname(dedupeReportPath), { recursive: true });
    fs.writeFileSync(
      dedupeReportPath,
      JSON.stringify({ summary, entries: dedupeOut }, null, 2),
      "utf8",
    );
    console.log(JSON.stringify({ wroteDedupeReport: dedupeReportPath, rowCount: dedupeOut.length }, null, 2));
  }
    ...
    JSON.stringify(
      {
        ...
        skippedInferiorIncoming: run.skippedInferiorIncoming,
        dryRunSkippedInferior: run.dryRunSkippedInferior,
