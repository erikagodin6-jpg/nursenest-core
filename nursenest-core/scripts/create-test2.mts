...
// Create user only (no subscription nested — add subscription separately to handle stripeSubscriptionId requirement)
...
await p.subscription.create({
  data: {
    userId: user.id,
    status: "ACTIVE",
    stripeSubscriptionId: `e2e_cnple_test_${Date.now()}`,
    planTier: "NP",
    planCode: "NP_CA_E2E_TEST",
