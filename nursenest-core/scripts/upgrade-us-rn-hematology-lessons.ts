...
const REQUIRED_HEME_TOPICS: Array<{ label: string; keys: string[] }> = [
  { label: "anemia types", keys: ["anemia", "iron deficiency", "b12", "folate", "hemolytic", "aplastic"] },
  { label: "clotting disorders", keys: ["clot", "coagul", "thromb", "platelet", "dic"] },
  { label: "transfusions", keys: ["transfusion", "blood product"] },
  { label: "transfusion reactions", keys: ["transfusion reaction", "reaction"] },
];

const HEME_FOCUS_KEYS = [
  "hemat",
  "anemia",
  "iron deficiency",
  "b12",
...
const HEME_EXCLUDE_KEYS = ["obstetric", "labor", "postpartum", "fetal"];

function isHemeText(text: string): boolean {
  const t = text.toLowerCase();
  if (HEME_EXCLUDE_KEYS.some((k) => t.includes(k))) return false;
  return HEME_FOCUS_KEYS.some((k) => t.includes(k));
}
...
async function main() {
  ...
  const hemeRows = allRows.filter((r) => isHemeText(`${r.title} ${r.topic} ${r.bodySystem} ${r.slug}`));
  const corpus = hemeRows.map((r) => `${r.title} ${r.topic} ${r.bodySystem} ${r.slug}`.toLowerCase()).join("\n");
  const missingTopics = REQUIRED_HEME_TOPICS.filter((t) => !t.keys.some((k) => corpus.includes(k.toLowerCase()))).map((t) => t.label);
    ...
    JSON.stringify(
      {
        ...
        missingHematologyTopics: missingTopics,
      },
      null,
      2,
    ),
