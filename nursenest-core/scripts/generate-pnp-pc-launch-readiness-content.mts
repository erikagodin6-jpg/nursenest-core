...
const PATHWAY_ID = "us-np-pnp-pc";
const SOURCE_LESSON_PATHWAY_ID = "us-np-fnp";
const SOURCE = "pnp-pc-launch-readiness-sprint";
const CATEGORY_SLUG = "pnp-pc-launch-readiness";
const DECK_SLUG = "pnp-pc-clinical-reasoning-launch-deck";
const LESSON_CATALOG = resolve(process.cwd(), "src/content/pathway-lessons/np-parity-expansion-catalog.json");
const PNP_PC_INCLUDED_LESSON_RE =
  /\b(pediatric|pediatrics|child|children|adolescent|infant|newborn|toddler|school-age|development|growth|immunization|vaccine|asthma|bronchiolitis|otitis|strep|rash|eczema|dermatitis|gastroenteritis|adhd|autism|behavioral|mental|endocrine|diabetes|family|caregiver)\b/i;
const PNP_PC_PRIORITY_DOMAIN_IDS = new Set([
  "health-promotion",
  "preventive-care",
  "pediatrics",
  "mental-health",
...
const PNP_PC_OVERLAY_DOMAINS: Domain[] = [
  domain("well-child-care", "Well Child Care", "Pediatrics", "Health Promotion and Prevention", "Well child preventive care", "Preventive care", "a child presents for a well visit with caregiver concerns and incomplete records", "growth percentile shift, delayed immunizations, developmental concern, and caregiver health literacy shape priorities", "normal variation versus developmental delay versus chronic condition versus safety concern", "review growth curves, developmental surveillance, immunization status, nutrition, sleep, safety, and family context", "build an age-specific anticipatory guidance and follow-up plan with caregiver teach-back", "avoid medication unless diagnosis, age, dose, and caregiver administration capacity support it", "checking only milestone recall without interpreting growth, caregiver concern, or risk context", "treating the well visit as a checklist instead of risk-based pediatric assessment", "integrate growth, development, prevention, and family capacity into one plan", "missed developmental delay, vaccine gap, injury risk, and caregiver misunderstanding"),
  domain("growth-development", "Growth & Development", "Pediatrics", "Health Promotion and Prevention", "Growth and developmental screening", "Assessment", "an infant, toddler, school-age child, or adolescent has growth or developmental concerns", "crossing percentiles, regression, failed screen, caregiver concern, and school function determine urgency", "normal variant versus developmental delay versus endocrine/nutritional disease versus neurodevelopmental disorder", "plot trends, use validated screening, assess nutrition/sleep/family context, and identify red flags", "refer early when screening or regression is abnormal while supporting family-centered follow-up", "avoid sedating or appetite-altering medications until growth and development implications are reviewed", "reassuring because one milestone is only slightly delayed", "single-milestone thinking without trend or validated screening", "developmental concern is interpreted through trajectory, function, and caregiver observation", "delayed referral, missed autism/ADHD, failure to thrive, and family distress"),
  domain("immunizations", "Immunizations", "Pediatrics", "Health Promotion and Prevention", "Pediatric immunization decisions", "Preventive care", "a child is behind on vaccines or has a caregiver question about safety", "age, previous doses, contraindications, immune status, and outbreak exposure determine catch-up planning", "true contraindication versus precaution versus vaccine hesitancy versus catch-up schedule need", "verify records, identify contraindications, build catch-up schedule, and use motivational counselling", "administer indicated vaccines or arrange catch-up with clear safety-netting and documentation", "defer vaccines only for true contraindications or appropriate precautions", "delaying all vaccines because the caregiver is unsure", "confusing hesitancy with a contraindication", "protect the child and community while addressing caregiver concerns respectfully", "preventable infection, missed school requirements, and loss of trust"),
  domain("asthma-respiratory", "Respiratory Disorders", "Respiratory", "Clinical Assessment and Diagnosis", "Asthma and respiratory triage", "Management reasoning", "a child has cough, wheeze, respiratory distress, or recurrent nighttime symptoms", "work of breathing, oxygenation, frequency of rescue use, triggers, age, and feeding tolerance change severity", "viral bronchiolitis versus asthma exacerbation versus pneumonia versus foreign body or anaphylaxis", "assess severity, oxygenation, hydration, trigger history, and need for urgent referral", "treat by severity with bronchodilator/controller planning, spacer teaching, and follow-up", "choose medication by age, severity, technique, and caregiver ability; monitor side effects and response", "prescribing antibiotics or cough suppressants before assessing respiratory effort", "symptom-label prescribing without pediatric severity assessment", "pediatric respiratory decisions start with work of breathing and hydration, not diagnosis labels alone", "hypoxia, respiratory fatigue, dehydration, and medication misuse"),
  domain("pediatric-infection", "Pediatric Infectious Disease", "Pediatrics", "Clinical Assessment and Diagnosis", "Common pediatric infections", "Differential diagnosis", "a child presents with fever, sore throat, ear pain, rash, vomiting, or poor intake", "age, immunization status, hydration, toxic appearance, exposure, and localizing signs drive risk", "viral illness versus bacterial infection versus sepsis/meningitis versus dehydration complication", "assess appearance, hydration, fever pattern, exposure, and indications for testing or referral", "use supportive care, targeted testing, or antibiotics only when criteria support the diagnosis", "select antibiotic by age, allergy, likely organism, resistance risk, and weight-based dosing", "treating every fever with antibiotics to reassure the caregiver", "caregiver pressure overriding diagnostic criteria and stewardship", "distinguish well-appearing viral illness from bacterial or toxic presentations", "sepsis delay, dehydration, antimicrobial resistance, and adverse drug event"),
  domain("adhd-behavioral", "ADHD and Behavioral Health", "Mental Health", "Clinical Assessment and Diagnosis", "Behavioral and neurodevelopmental care", "Assessment", "a school-age child has inattention, impulsivity, sleep issues, anxiety, or school impairment", "symptoms across settings, sleep, trauma, learning concerns, caregiver report, and teacher input change diagnosis", "ADHD versus anxiety/depression versus sleep disorder versus learning disorder versus autism spectrum disorder", "use validated scales, assess impairment across settings, screen comorbidity, and involve family/school", "plan behavioural supports, school coordination, and medication when criteria and monitoring are satisfied", "choose stimulant or nonstimulant therapy with growth, appetite, sleep, cardiovascular, and misuse monitoring", "starting medication after one caregiver report without school or comorbidity assessment", "diagnosing ADHD without cross-setting impairment and differential diagnosis", "PMH and developmental symptoms require context, function, and longitudinal follow-up", "misdiagnosis, medication adverse effects, school failure, and family conflict"),
  domain("pediatric-pharmacology", "Pediatric Pharmacology", "Pharmacology", "Pharmacotherapeutics", "Weight-based pediatric prescribing", "Prescribing", "a child needs antibiotics, asthma medication, ADHD treatment, dermatologic therapy, endocrine therapy, or emergency medication", "weight, age, formulation, allergies, renal/hepatic function, caregiver ability, and monitoring needs determine safe prescribing", "appropriate pediatric therapy versus contraindicated agent versus dose/formulation/monitoring problem", "confirm weight, diagnosis, allergy, severity, formulation access, and family education needs", "prescribe with weight-based dosing, max-dose checks, monitoring, side-effect counselling, and follow-up", "calculate dose by current weight and condition; teach caregiver administration, side effects, interactions, and return precautions", "using adult dose shortcuts or ignoring formulation and caregiver administration barriers", "right medication but wrong pediatric dose, formulation, or monitoring", "pediatric prescribing is diagnosis plus weight-based dosing plus caregiver execution", "toxicity, treatment failure, dosing error, and avoidable emergency visit"),
];

const PNP_PC_PRIORITY_DOMAINS = [
  ...PNP_PC_OVERLAY_DOMAINS,
  ...PRIORITY_DOMAINS.filter((domain) => PNP_PC_PRIORITY_DOMAIN_IDS.has(domain.id)),
];
...
function loadLessons(): CatalogLesson[] {
  ...
  const lessons = (Array.isArray(entry) ? entry : entry?.lessons ?? []).filter((lesson) => {
    ...
    return PNP_PC_INCLUDED_LESSON_RE.test(text);
  });
...
function lessonDomain(lesson: CatalogLesson, index: number): Domain {
  ...
  const direct =
    PNP_PC_PRIORITY_DOMAINS.find((d) => text.includes(d.id.replace(/-/g, " "))) ??
    PNP_PC_PRIORITY_DOMAINS.find((d) => text.includes(d.label.toLowerCase())) ??
    PNP_PC_PRIORITY_DOMAINS.find((d) => text.includes(d.system.toLowerCase()));
  return direct ?? PNP_PC_PRIORITY_DOMAINS[index % PNP_PC_PRIORITY_DOMAINS.length]!;
}
...
function teachingRationale(lesson: CatalogLesson, domain: Domain, correct: string): string {
  return [
    ...
    `Why alternatives are tempting: ${domain.tempting}. The PNP-PC trap is ${domain.trap}.`,
...
function optionRationales(optionMap: Record<string, string>, correctLetter: string, domain: Domain): Record<string, string> {
  return Object.fromEntries(
    Object.entries(optionMap).map(([letter, text]) => {
      ...
      return [
        letter,
        `${letter} is incorrect. ${text} is tempting because ${domain.tempting}, but it misses ${domain.cue}, tests the PNP-PC trap of ${domain.trap}, and fails to apply this NP reasoning: ${domain.reasoning}.`,
      ];
    }),
  );
}
...
function buildMcq(lesson: CatalogLesson, domain: Domain, localIndex: number, globalIndex: number): QuestionRow {
  ...
  const stem = `PNP-PC ${domain.label} item ${localIndex + 1}. A nurse practitioner sees ${domain.presentation} while applying ${lesson.title}. Key cue: ${domain.cue}. Which plan best demonstrates advanced NP clinical reasoning?`;
  return questionRow({
    id: `pnp-pc-${slug(lesson.slug)}-q-${String(localIndex + 1).padStart(3, "0")}`,
    lesson,
    domain,
    stem,
    questionType: "MCQ",
...
function buildSata(lesson: CatalogLesson, domain: Domain, localIndex: number, globalIndex: number): QuestionRow {
  ...
  return questionRow({
    ...
    rationale: `A-D are correct because PNP-PC-level care requires differential diagnosis, targeted diagnostic interpretation, management reasoning, and prescribing safety. E is premature closure. F is low-value testing unless each test changes probability, safety, treatment, or follow-up.`,
    difficulty: difficulty(globalIndex),
...
function buildMatrix(lesson: CatalogLesson, domain: Domain, localIndex: number, globalIndex: number): QuestionRow {
  const rows = [
    ...
    { id: "prescribing", label: `Medication choice affected by weight, age, formulation, renal function, interactions, or monitoring`, answer: "Prescribing safety" },
...
function buildOrdered(lesson: CatalogLesson, domain: Domain, localIndex: number, globalIndex: number): QuestionRow {
  ...
  return questionRow({
    ...
    rationale: `PNP-PC reasoning moves from differential diagnosis to diagnostic confirmation, management decision, and prescribing/monitoring. The trap is prescribing or reassuring before diagnostic risk is resolved.`,
    difficulty: difficulty(globalIndex),
...
function buildClinicalManagement(lesson: CatalogLesson, domain: Domain, localIndex: number, globalIndex: number): QuestionRow {
  ...
  return questionRow({
    ...
    lesson,
    domain,
    stem: `Clinical management. Which management plan is safest for ${domain.presentation} in a PNP-PC-style ${lesson.title} encounter?`,
    questionType: "MCQ",
...
function buildDiagnosticInterpretation(lesson: CatalogLesson, domain: Domain, localIndex: number, globalIndex: number): QuestionRow {
  ...
  return questionRow({
    ...
    lesson,
    domain,
    stem: `Diagnostic interpretation. A result is available during ${lesson.title}. Which interpretation best reflects PNP-PC-level NP reasoning?`,
    questionType: "MCQ",
...
function questionRow(input: {
  ...
  return {
    ...
    clinicalPearl: `PNP-PC pearl: ${input.domain.cue} should change the differential, diagnostic plan, management plan, prescribing safety check, or follow-up interval.`,
    examStrategy: "For PNP-PC, avoid RN-level recall. Identify the differential, interpret targeted data, prescribe safely, and decide whether outpatient management is safe.",
    ...
    countryCode: "US",
    regionCode: "US",
    licensingBody: "Pediatric Primary Care NP",
    languageCode: "en",
    ...
    referenceSource: "NurseNest PNP-PC lesson-derived launch readiness sprint",
...
function buildQuestions(lessons: CatalogLesson[]): QuestionRow[] {
  ...
  while (rows.length < QUESTION_TARGET) {
    ...
    const domain = PNP_PC_PRIORITY_DOMAINS[extra % PNP_PC_PRIORITY_DOMAINS.length]!;
...
function flashcardRow(lesson: CatalogLesson, domain: Domain, index: number, categoryId: string, deckId: string, sourceSuffix: string): FlashcardRow {
  ...
  const front =
    mode === "case"
      ? `PNP-PC case: In ${lesson.title}, how should the NP reason through ${domain.presentation} when the cue is ${domain.cue}?`
      : `PNP-PC ${mode}: What is the key NP-level decision in ${lesson.title} when ${domain.cue} is present?`;
  const back = [
  ...
  return {
    id: `pnp-pc-fc-${String(index + 1).padStart(5, "0")}`,
    front,
    back,
    ...
    sourceKey: `pnp-pc:flashcard:${sourceSuffix}`,
    examItemKind: null,
  };
}
...
async function createPracticeExams(
  ...
  for (let i = 1; i <= EXAM_TARGET; i++) {
    ...
    const examId = `exam_pnp-pc_launch_${padded}`;
    const tag = `exam-preset-pnp-pc-launch-${padded}`;
    await prisma.exam.upsert({
      where: { id: examId },
      update: { title: `PNP-PC Blueprint Practice Exam ${i}`, status: ContentStatus.PUBLISHED },
      create: {
        id: examId,
        title: `PNP-PC Blueprint Practice Exam ${i}`,
        country: CountryCode.US,
        tier: TierCode.NP,
        examFamily: ExamFamily.NP,
        status: ContentStatus.PUBLISHED,
...
async function main(): Promise<void> {
  ...
  try {
    const category = await prisma.category.upsert({
      ...
      update: { name: "PNP-PC Launch Readiness", topicCode: "pnp-pc-launch-readiness" },
      create: { slug: CATEGORY_SLUG, name: "PNP-PC Launch Readiness", topicCode: "pnp-pc-launch-readiness" },
    });
    const deck = await prisma.flashcardDeck.upsert({
      where: { slug: DECK_SLUG },
      update: {
        title: "PNP-PC Advanced NP Clinical Reasoning Deck",
        description: "Lesson-derived Pediatric Primary Care NP flashcards focused on pediatric assessment, growth and development, immunizations, pediatric pharmacology, family education, prevention, and clinical reasoning.",
        country: CountryCode.US,
        tier: TierCode.NP,
        examFamily: ExamFamily.NP,
    ...
    console.log(`Publishing ${questions.length} PNP-PC questions...`);
    ...
    console.log(`Publishing ${flashcards.length} PNP-PC flashcards...`);
    ...
    const pool = await prisma.examQuestion.findMany({
      where: {
        ...
        exam: { in: ["PNP-PC", "NP-PNP-PC", "Pediatric-PC-NP", "NP"] },
    ...
    console.log(`PNP-PC launch readiness content published: questions=${questions.length}, flashcards=${flashcards.length}, adaptivePool=${pool.length}, practiceExams=${EXAM_TARGET}`);
  } finally {
