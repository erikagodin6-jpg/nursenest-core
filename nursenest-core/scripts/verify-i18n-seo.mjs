...
const lines = [
  ...
  "Incomplete French surfaces must stay out of sitemap and completed-alternate hreflang clusters.",
  "",
];
fs.writeFileSync(path.join(REPORT_DIR, "i18n-seo-verification.md"), lines.join("\n"));

const failures = indexedIncomplete.length + missingSeo.length + missingJsonLd.length;
if (failures > 0) {
