...
For **deploy-time drift** (same host visible but wrong password, rotated secret, pooler swap), see **`nursenest-core/docs/database-url-drift-audit.md`** — boot/connectivity logs and a **SHA-256 prefix fingerprint** of the full URL (password never printed).
