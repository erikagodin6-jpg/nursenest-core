...
| **Build** | `npm run build` (then on DO: `npm run build:deploy`) | CI / DO / Vercel | On App Platform, `heroku-postbuild` runs `next build` before cache snapshot; the buildpack’s second `npm run build` skips duplicate compile on DO. `build:deploy` → `build:deploy:postbuild` verifies standalone, syncs static, **`post-build-prune`** (keeps `.next/cache` by default; see `.do/app-nursenest-core-next.yaml`). Default **`RUN_HEAVY_BUILD_TASKS=false`** (see `next.config.ts`). |
...
- **Node heap (build):** DigitalOcean **BUILD_TIME** `NODE_OPTIONS=--max-old-space-size=4096` in `.do/app-nursenest-core-next.yaml` caps **all** build-phase Node processes (npm install, postbuild, wrapper). `build:compile` sets `NODE_OPTIONS=--max-old-space-size=${BUILD_NODE_MAX_OLD_SPACE_SIZE_MB:-4096}` **without** inheriting `${NODE_OPTIONS:-…}` so a UI-wide 6144 MB flag cannot apply to the wrapper process. `run-next-prod-build.mjs` still normalizes the Next child env. Remove duplicate / higher `NODE_OPTIONS` from the App **UI** if merge keeps 6144. Runtime: no `NODE_OPTIONS` in spec for RUN_TIME. `typecheck` uses a higher cap locally.
