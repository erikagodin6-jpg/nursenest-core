...
| **Build** | `npm run build` (then on DO: `npm run build:deploy`) | CI / DO / Vercel | Node buildpack runs `build`; `build:deploy` verifies standalone, syncs static, prunes `.next/cache` — **no** second `next build` (see `.do/app-nursenest-core-next.yaml`). Default **`RUN_HEAVY_BUILD_TASKS=false`** (see `next.config.ts`). |
