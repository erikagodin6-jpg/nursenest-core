...
import { readMergedBundleFromNextPublicI18n, writeMergedBundleToNextShardTree } from "./lib/next-public-i18n-bundle";
import { APP_ROOT, CLIENT_PUBLIC_I18N_DIR, NEXT_PUBLIC_I18N_SHARD_ROOT, REPO_ROOT } from "./repo-root";
...
export function mergeMarketingIntoI18n(): void {
  ...
  const clientOut = CLIENT_PUBLIC_I18N_DIR;
  const nextOut = NEXT_PUBLIC_I18N_SHARD_ROOT;
  ...
  for (const lang of I18N_LANGUAGES) {
    ...
    writeMergedBundleToNextShardTree(nextOut, lang, merged);
    const roundTrip = readMergedBundleFromNextPublicI18n(nextOut, lang);
    if (!roundTrip) {
      throw new Error(`[i18n] merge: could not read back Next shard tree for ${lang}`);
    }
    const mk = Object.keys(merged);
    if (mk.length !== Object.keys(roundTrip).length) {
      throw new Error(`[i18n] merge: Next shard key count mismatch for ${lang}`);
    }
    for (const k of mk) {
      if (merged[k] !== roundTrip[k]) {
        throw new Error(`[i18n] merge: Next shard value mismatch for ${lang} key ${k.slice(0, 120)}`);
      }
    }
  }

  const enKeys = Object.keys(mergedByLang["en"] ?? {}).sort();
  const keyTsPath = path.join(APP_ROOT, "src/lib/i18n/marketing-message-keys.generated.ts");
  const keyTsBody =
    `// Generated by script/merge-marketing-i18n.ts — do not edit.\n` +
    `export const MARKETING_MESSAGE_KEYS = [\n${enKeys.map((k) => `  ${JSON.stringify(k)},`).join("\n")}\n] as const;\n\n` +
    `export type MarketingMessageKey = (typeof MARKETING_MESSAGE_KEYS)[number];\n`;
  writeFileSync(keyTsPath, keyTsBody, "utf8");
