/**
 * Build public/i18n/{locale}.json with full key parity to en.json (Lingva: en → locale).
 * Resumes from .cache/translate-{locale}-lingva-cache.json
 *
 * Usage: LOCALE=tl node script/build-locale-i18n-lingva.mjs
 * Env: LINGVA_CONCURRENCY (default 2), LINGVA_DELAY_MS (default 350), TRANSLATE_LIMIT (optional cap)
 */
...
if (!/^[a-z]{2}(-[a-z]{2})?$/i.test(LOCALE)) {
  console.error("Set LOCALE=tl (or zh-tw, etc.)");
  process.exit(1);
}
const localeLower = LOCALE.toLowerCase();

const enPath = path.join(root, "nursenest-core/public/i18n/en.json");
const outPath = path.join(root, `nursenest-core/public/i18n/${localeLower}.json`);
const clientPath = path.join(root, `client/public/i18n/${localeLower}.json`);
const cacheDir = path.join(root, ".cache");
const cachePath = path.join(cacheDir, `translate-${localeLower}-lingva-cache.json`);
...
async function main() {
  ...
  const partial = existsSync(outPath) ? loadJson(outPath) : {};
  ...
  console.log(
    `Locale ${localeLower}: keys ${enKeys.length}, to translate ${capped.length}/${list.length}, cached ${Object.keys(cache).length}`,
  );
